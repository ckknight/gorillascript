...
class MacroContext
  ...
  def error(message as String, node as LispyNode|null)
    throw @parser.build-error message, node or @index
  
  def scope() -> @parser.scope.peek()
    
  def line(node)
    let index = if node instanceof LispyNode
      node.index
    else
      @index
  ...
  def custom(name as String, ...data as [LispyNode])
    LispyNode.InternalCall(\custom, @index, @scope(),
      LispyNode.Value @index, name
      ...data).reduce(@parser)
  def noop()
    LispyNode.Symbol.nothing @index
  def block(nodes as [LispyNode])
    LispyNode.InternalCall(\block, @index, @scope(), ...nodes).reduce(@parser)
  def if(test as LispyNode = LispyNode.Symbol.nothing(0), when-true as LispyNode = LispyNode.Symbol.nothing(0), when-false as LispyNode = LispyNode.Symbol.nothing(0))
    LispyNode.InternalCall(\if, @index, @scope(),
      @do-wrap(test)
      when-true
      when-false).reduce(@parser)
  def switch(topic as LispyNode = LispyNode.Symbol.nothing(0), cases as [], default-case as LispyNode|null)
    let args = [
      @do-wrap(topic)
    ]
    for case_ in cases
    ...
    LispyNode.InternalCall(\switch, @index, @scope(),
      ...args).reduce(@parser)
  def for(init as LispyNode|null, test as LispyNode|null, step as LispyNode|null, body as LispyNode = LispyNode.Symbol.nothing(0))
    LispyNode.InternalCall(\for, @index, @scope(),
      @do-wrap(init)
      @do-wrap(test)
      @do-wrap(step)
      body).reduce(@parser)
  def for-in(key as Tmp|Ident, object as LispyNode = LispyNode.Symbol.nothing(0), body as LispyNode = LispyNode.Symbol.nothing(0))
    LispyNode.InternalCall(\for-in, @index, @scope(),
      key
      @do-wrap(object)
      body).reduce(@parser)
  def try-catch(try-body as LispyNode = LispyNode.Symbol.nothing(0), catch-ident as LispyNode = LispyNode.Symbol.nothing(0), catch-body as LispyNode = LispyNode.Symbol.nothing(0))
    LispyNode.InternalCall(\try-catch, @index, @scope(),
      try-body
      catch-ident
      catch-body).reduce(@parser)
  def try-finally(try-body as LispyNode = LispyNode.Symbol.nothing(0), finally-body as LispyNode = LispyNode.Symbol.nothing(0))
    LispyNode.InternalCall(\try-finally, @index, @scope(),
      try-body
      finally-body).reduce(@parser)
  def assign(left as LispyNode = LispyNode.Symbol.nothing(0), op as String, right as LispyNode = LispyNode.Symbol.nothing(0))
    LispyNode.Call(@index, @scope()
      LispyNode.Symbol.assign[op] @index
      left
      @do-wrap(right)).reduce(@parser)
  def binary(left as LispyNode = LispyNode.Symbol.nothing(0), op as String, right as LispyNode = LispyNode.Symbol.nothing(0))
    LispyNode.Call(@index, @scope()
      LispyNode.Symbol.binary[op] @index
      @do-wrap(left)
      @do-wrap(right)).reduce(@parser)
  def binary-chain(op as String, nodes as [LispyNode])
    if nodes.length == 0
  ...
  def unary(op as String, node as LispyNode = LispyNode.Symbol.nothing(0))
    LispyNode.Call(@index, @scope()
      LispyNode.Symbol.unary[op] @index
      @do-wrap(node)).reduce(@parser)
  def throw(node as LispyNode = LispyNode.Symbol.nothing(0))
    LispyNode.InternalCall(\throw, @index, @scope(),
      @do-wrap(node)).reduce(@parser)
  def return(node as LispyNode = LispyNode.Symbol.nothing(0))
    LispyNode.InternalCall(\return, @index, @scope(),
      @do-wrap(node)).reduce(@parser)
  def yield(node as LispyNode = LispyNode.Symbol.nothing(0))
  ...
  def is-const(node) -> is-void! node or (node instanceof LispyNode and @real(node).is-const())
  def value(node)
    if is-void! node
      void
  ...
  def is-node(node) -> node instanceof LispyNode
  ...
  def context-call(func as LispyNode, context as LispyNode, ...args as [LispyNode])
  ...
  def new(func as LispyNode, ...args as [LispyNode])
  ...
  def call(func as LispyNode, args as [LispyNode] = [])
    LispyNode.Call(func.index, @scope(),
      func
      ...(for arg in args; @do-wrap(arg))).reduce(@parser)
  
  def func(mutable params as [LispyNode], body as LispyNode, bound as (LispyNode|Boolean) = false, as-type as LispyNode|void, is-generator as Boolean)
  ...
  def param(ident as LispyNode, default-value as LispyNode|null, spread as Boolean, is-mutable as Boolean, as-type as LispyNode|null)
      ...
      if pair instanceof LispyNode
        ...
        LispyNode.InternalCall(\array, @index, @scope(),
          ...
          ...(if property
            ...
            else if property instanceof LispyNode and property.is-const-type(\string)
              [property]
            else
  ...
  def type(node)
    if is-string! node
      ...
      throw Error "Can only retrieve type from a String or LispyNode, got $(typeof! node)"
  ...
  def cache(node as LispyNode, init, name as String = \ref, save as Boolean)
    @maybe-cache node, (#(set-node, node, cached)
      if cached
        init.push set-node
      node), name, save
  
  def maybe-cache(mutable node as LispyNode, func, name as String = \ref, save as Boolean)
    node := @macro-expand-1(node)
    if @is-complex node
  ...
  def maybe-cache-access(mutable node as LispyNode, func, parent-name as String = \ref, child-name as String = \ref, save as Boolean)
    node := @macro-expand-1 node
    if @is-access(node)
  ...
  def walk(node as LispyNode|void|null, func as LispyNode -> LispyNode)
    if node?
      walk@ this, node, func
    else
      node
  ...
  def is-statement(mutable node)
    ...
    node instanceof LispyNode and node.is-statement()
