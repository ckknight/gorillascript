(function (GLOBAL) {
  ...
  var __bind, __cmp, __compose, __create, __curry, __defer,
      ...
      LessThanChar, Letter, LicenseComment, Line, LispyNode, Literal, Logic,
      LSymbol, MacroAccessNode, MacroBody, MacroConstNode, MacroContext,
      ...
      UnicodeEscapeSequence, unique, unusedCaches, UseMacro, Value, VarNode,
      word, wordOrSymbol, YieldNode, Zero;
  _this = this;
  ...
  RadixNumber = cache((function () {
    ...
    return function (parser, index) {
      ...
      return Box(trailing.index, Value(index, value));
    };
  }()));
  ...
  IdentifierNameConst = cache(function (parser, index) {
    ...
    name = Name(parser, index);
    if (name) {
      return Box(name.index, Value(index, name.value));
    }
  });
  ...
  SingleStringLiteral = cache((_ref = sequential(
    ...
    return Value(index, codesToString(codes));
  })(_ref)));
  ...
  function doubleStringLiteralHandler(parts, parser, index) {
    ...
    for (_arr = __toArray(parts), _i = 0, _len = _arr.length; _i < _len; ++_i) {
      ...
      if (typeof part === "number") {
        ...
        stringParts.push(Value(index, codesToString(currentLiteral)));
        currentLiteral = [];
  ...
  function tripleStringHandler(x, parser, index) {
    ...
    for (i = 0, _len = stringParts.length; i < _len; ++i) {
      ...
      if (typeof part === "string") {
        stringParts[i] = Value(index, part);
      }
    }
    return stringParts;
  }
  ...
  function CustomConstantLiteral(parser, index) {
    ...
    if (parser.inAst.peek()) {
      ...
      return Box(currentIndex, Value(index, current));
    }
  }
  ...
  IdentifierOrSimpleAccessStart = oneOf(
    ...
    (_ref = sequential(
      ...
      return parser.Access(
        index,
        parser.Access(index, parent, Value(index, "prototype")),
        child
      );
    })(_ref)),
    (_ref = sequential(
  ...
  IdentifierOrSimpleAccessPart = mutate(function (_p, parser, childIndex) {
    ...
    return function (parent, parser, index) {
      return parser.Access(
        index,
        isProto
          ? parser.Access(index, parent, Value(childIndex, "prototype"))
          : parent,
        child
      );
    };
  ...
  ConstObjectKey = oneOf(
    ...
    mutate(function (node, parser, index) {
      return Value(index, String(node.constValue()));
    })(NumberLiteral),
    IdentifierNameConst
  );
  ...
  SingularObjectKey = oneOf(
    ...
    (_ref = sequential(
      ...
      if (node.isConst() && !node.isConstType("string")) {
        key = Value(index, String(node.value));
      } else {
        key = node;
      }
    ...
    macroSyntaxTypes = {
      syntax: function (index, params, body, options, stateOptions, translator) {
        ...
        body = this.Block(index, [
          ...
          this.Assign(index, macroNameIdent, "=", this.Access(index, macroFullDataIdent, Value(index, "macroName"))),
          this.Var(index, macroDataIdent, false),
          this.Assign(index, macroDataIdent, "=", this.Access(index, macroFullDataIdent, Value(index, "macroData")))
        ].concat(
          (function () {
            var _arr, _arr2, _i, _len, param;
            for (_arr = [], _arr2 = __toArray(params), _i = 0, _len = _arr2.length; _i < _len; ++_i) {
              ...
              if (param instanceof SyntaxParamNode) {
                ...
                _arr.push(_this.Block(index, [
                  ...
                  _this.Assign(index, param.ident, "=", _this.Access(index, macroDataIdent, Value(index, param.ident.name)))
                ]));
              }
            }
            return _arr;
          ...
          (function () {
            ...
            for (_arr = [], _arr2 = __toArray(params), i = 0, _len = _arr2.length; i < _len; ++i) {
              ...
              if (param instanceof ParamNode) {
                ...
                _arr.push(_this.Block(index, [
                  _this.Var(index, param.ident, true),
                  _this.Assign(index, param.ident, "=", _this.Access(index, macroDataIdent, Value(index, i)))
                ]));
              }
            }
            return _arr;
        ...
        body = this.Block(index, (function () {
          ...
          for (_arr = [], _arr2 = ["left", "op", "right"], _i = 0, _len = _arr2.length; _i < _len; ++_i) {
            ...
            _arr.push(_this.Block(index, [
              ...
              _this.Assign(index, ident, "=", _this.Access(index, macroDataIdent, Value(index, name)))
            ]));
          }
          return _arr;
        }()).concat([body]));
