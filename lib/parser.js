...
break;
  ...
  State = (function () {
    ...
    function calcParam(param) {
      var calced, multiplier, name, string;
      if (param instanceof IdentNode) {
        name = param.name;
        if (__owns(macroSyntaxIdents, name)) {
          return macroSyntaxIdents[name];
        } else {
          return function (o) {
            if (__owns(macroSyntaxIdents, name)) {
              return macroSyntaxIdents[name].call(this, o);
            } else {
              return this.error("Unknown type ident: " + __strnum(name));
            }
          };
        }
    ...
    _proto.macroSyntax = function (index, type, params, body, options) {
      var _ref, _this, handler, macroId, macros, rule;
      _this = this;
      if (!__isArray(params)) {
        throw TypeError("Expected params to be a Array, got " + __typeof(params));
      }
      ...
      macroId = (function () {
        ...
        case DEFINE_SYNTAX:
          if (__owns(macroSyntaxIdents, options.name)) {
            throw Error("Cannot override already-defined syntax: " + __strnum(options.name));
          }
          macroSyntaxIdents[options.name] = named(
