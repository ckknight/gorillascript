...
break;
  ...
  Node.Binary = BinaryNode = (function (_super) {
    ...
    _proto._reduce = (function () {
      ...
      leftConstOps = {
        ...
        "+": function (x, y) {
          if (x.constValue() === 0 && y.type().isSubsetOf(types.number)) {
            return UnaryNode(this.startIndex, this.endIndex, "+", y);
      ...
      rightConstOps = {
        ...
        "+": function (x, y) {
          if (y.constValue() === 0 && x.type().isSubsetOf(types.number)) {
            return UnaryNode(this.startIndex, this.endIndex, "+", x);
          } else if (typeof y.constValue() === "number" && __num(y.value) < 0 && x.type().isSubsetOf(types.number)) {
            return BinaryNode(
              this.startIndex,
              this.endIndex,
              x,
          ...
          } else if (y.constValue() === "" && x.type().isSubsetOf(types.string)) {
            return x;
          }
        },
        "-": function (x, y) {
          if (y.constValue() === 0) {
            return UnaryNode(this.startIndex, this.endIndex, "+", x);
          } else if (typeof y.constValue() === "number" && __num(y.constValue()) < 0 && x.type().isSubsetOf(types.number)) {
            return BinaryNode(
              this.startIndex,
              this.endIndex,
              x,
  ...
  Node.String = StringNode = (function (_super) {
    ...
    _proto._reduce = function () {
      ...
      if (__num(segments.length) > 1 && segments[0].isConst() && segments[0].constValue() === "" && (segments[1].type().isSubsetOf(types.string) || __num(segments.length) > 2 && segments[2].type().isSubsetOf(types.string))) {
        segments.shift();
      }
      result = segments[0];
  ...
  Node.Unary = UnaryNode = (function (_super) {
    ...
    _proto.type = (function () {
      ...
      return function () {
        var _ref;
        return (__owns(ops, _ref = this.op) ? ops[_ref] : void 0) || types.any;
      };
    }());
    _proto._reduce = (function () {
