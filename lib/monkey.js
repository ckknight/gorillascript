...
if (require.extensions) {
  require.extensions[".ms"] = function (module, filename) {
    var content;
    content = compile(fs.readFileSync(filename, "utf8"), { filename: filename });
...
exports["eval"] = function (source, options) {
  var _module, _require, _tmp, _tmp2, _tmp3, code, fun, k, Module, r, root, sandbox, Script, v;
  if (options == null) {
    options = {};
  }
  ...
  if (Script) {
    ...
    if (!sandbox.module && !sandbox.require) {
      ...
      for (_tmp = Object.getOwnPropertyNames(require), _tmp2 = 0, _tmp3 = __num(_tmp.length); _tmp2 < _tmp3; ++_tmp2) {
        r = _tmp[_tmp2];
        try {
...
exports.run = function (source, options) {
  ...
  if (path.extname(mainModule.filename) !== ".ms" || require.extensions) {
    return mainModule._compile(compile(source, options), mainModule.filename);
  } else {
