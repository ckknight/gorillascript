  ...
  module.exports = function (root, sources, coverageName) {
    ...
    function walker(node, parent, position) {
      ...
      if (file && sources[file] && __num(line) > 0) {
        ...
        if (!doneLines[line]) {
          if (parent instanceof ast.Func && (position === "param" || position === "name")) {
            return;
          } else if (parent instanceof ast.TryCatch && position === "catchIdent") {
            return;
          } else if (parent instanceof ast.ForIn && position === "key") {
            return;
          } else if (parent instanceof ast.Binary && parent.isAssign() && position === "left") {
            return;
          } else if (parent instanceof ast.Unary && parent.isAssign()) {
            return;
          } else if (parent instanceof ast.Switch && position === "caseNode") {
            return node;
          } else if ((parent instanceof ast.IfStatement || parent instanceof ast.IfExpression) && position === "test" && line === parent.whenTrue.pos.line) {
            return;
          } else if ((node instanceof ast.IfStatement || node instanceof ast.IfExpression) && line === node.whenTrue.pos.line) {
            return ast.If(
              pos,
              node.test,
              (_ref = walker(node.whenTrue, node, "whenFalse")) != null ? _ref : node.whenFalse.walk(walker),
              (_ref = walker(node.whenFalse, node, "whenFalse")) != null ? _ref : node.whenFalse.walk(walker),
              node.label
            );
          } else if (node instanceof ast.Binary && node.op === "." && parent instanceof ast.Call && position === "func") {
            return;
          } else if (node instanceof ast.Noop) {
            return;
          } else if (node instanceof ast.Func && node.body.pos.line === line) {
            return;
          } else if (node instanceof ast.Binary && node.isAssign() && node.right instanceof ast.Func && node.right.pos.line === line) {
