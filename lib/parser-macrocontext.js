(function () {
  ...
  var __create, __isArray, __owns, __slice, __throw, __toArray, __typeof, _ref,
      ...
      BinaryNode, BlockNode, CallNode, DefNode, EmbedWriteNode, EvalNode,
      ForInNode, ForNode, FunctionNode, IdentNode, IfNode, LispyNode,
  ...
  MacroContext = (function () {
    ...
    _MacroContext_prototype.isLabeledBlock = function (node) {
      node = this.real(node);
      if (node instanceof BlockNode || node instanceof IfNode || node instanceof SwitchNode || node instanceof ForNode || node instanceof ForInNode) {
        return node.label != null;
      } else if (node instanceof LispyNode && node.isCall && node.func.isInternal) {
        if (node.func.isTryFinally) {
          return node.args[2] != null;
        } else if (node.func.isTryCatch) {
        ...
        if (node.isCall) {
          ...
          if (func.isInternal) {
            if (func.isBreak || func.isContinue) {
              ...
              return node.args[2];
            } else if (func.isTryCatch) {
              return node.args[3];
            }
          }
        }
      } else if (node instanceof BlockNode || node instanceof IfNode || node instanceof SwitchNode || node instanceof ForNode || node instanceof ForInNode) {
        return node.label;
      }
      return null;
    };
