(function () {
  ...
  var __create, __isArray, __owns, __slice, __throw, __toArray, __typeof, _ref,
      ...
      ForNode, FunctionNode, IdentNode, IfNode, LispyNode, MacroAccessNode,
      MacroConstNode, MacroContext, Node, nodeToType, NothingNode, ObjectNode,
      ParamNode, RootNode, Scope, SpreadNode, SuperNode, SwitchNode,
  ...
  MacroContext = (function () {
    ...
    _MacroContext_prototype.isLabeledBlock = function (node) {
      node = this.real(node);
      if (node instanceof BlockNode || node instanceof IfNode || node instanceof SwitchNode || node instanceof ForNode) {
        return node.label != null;
      } else if (node instanceof LispyNode && node.isCall && node.func.isInternal) {
        if (node.func.isTryFinally) {
          return node.args[2] != null;
        } else if (node.func.isTryCatch || node.func.isForInNode) {
    ...
    _MacroContext_prototype.label = function (node) {
      ...
      } else if (node instanceof BlockNode || node instanceof IfNode || node instanceof SwitchNode || node instanceof ForNode) {
        return node.label;
      }
      return null;
    };
