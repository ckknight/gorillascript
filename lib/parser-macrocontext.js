(function () {
  ...
  var __create, __isArray, __owns, __slice, __throw, __toArray, __typeof, _ref,
      addParamToScope, Ident, LispyNode, MacroContext, nodeToType, Scope, Tmp,
      Type;
  ...
  Tmp = LispyNode.Symbol.tmp;
  Ident = LispyNode.Symbol.ident;
  function identity(x) {
    return x;
  }
  function retThis() {
  ...
  MacroContext = (function () {
    ...
    _MacroContext_prototype.noop = function () {
      return LispyNode.Symbol.nothing(this.index);
    };
    _MacroContext_prototype.block = function (nodes) {
      return LispyNode.InternalCall.apply(LispyNode, ["block", this.index, this.scope()].concat(nodes)).reduce(this.parser);
    };
    _MacroContext_prototype["if"] = function (test, whenTrue, whenFalse) {
      if (test == null) {
        test = LispyNode.Symbol.nothing(0);
      }
      if (whenTrue == null) {
        whenTrue = LispyNode.Symbol.nothing(0);
      }
      if (whenFalse == null) {
        whenFalse = LispyNode.Symbol.nothing(0);
      }
      return LispyNode.InternalCall(
        "if",
        this.index,
    ...
    _MacroContext_prototype["switch"] = function (topic, cases, defaultCase) {
      ...
      if (topic == null) {
        topic = LispyNode.Symbol.nothing(0);
      }
      if (defaultCase == null) {
        defaultCase = null;
      }
      ...
      return LispyNode.InternalCall.apply(LispyNode, ["switch", this.index, this.scope()].concat(args)).reduce(this.parser);
    };
    _MacroContext_prototype["for"] = function (init, test, step, body) {
      ...
      if (body == null) {
        body = LispyNode.Symbol.nothing(0);
      }
      return LispyNode.InternalCall(
        "for",
        this.index,
    ...
    _MacroContext_prototype.forIn = function (key, object, body) {
      if (object == null) {
        object = LispyNode.Symbol.nothing(0);
      }
      if (body == null) {
        body = LispyNode.Symbol.nothing(0);
      }
    ...
    _MacroContext_prototype.tryCatch = function (tryBody, catchIdent, catchBody) {
      if (tryBody == null) {
        tryBody = LispyNode.Symbol.nothing(0);
      }
      if (catchIdent == null) {
        catchIdent = LispyNode.Symbol.nothing(0);
      }
      if (catchBody == null) {
        catchBody = LispyNode.Symbol.nothing(0);
      }
      return LispyNode.InternalCall(
        "tryCatch",
        this.index,
    ...
    _MacroContext_prototype.tryFinally = function (tryBody, finallyBody) {
      ...
      if (finallyBody == null) {
        finallyBody = LispyNode.Symbol.nothing(0);
      }
      return LispyNode.InternalCall(
        "tryFinally",
        this.index,
    ...
    _MacroContext_prototype.assign = function (left, op, right) {
      if (left == null) {
        left = LispyNode.Symbol.nothing(0);
      }
      if (right == null) {
        right = LispyNode.Symbol.nothing(0);
      }
    ...
    _MacroContext_prototype.unary = function (op, node) {
      if (node == null) {
        node = LispyNode.Symbol.nothing(0);
      }
    ...
    _MacroContext_prototype.eq = function (alpha, bravo) {
      ...
      if (alpha instanceof LispyNode) {
        ...
        return bravo instanceof Ident && alpha.name === bravo.name;
      } else {
        return false;
      }
    };
    ...
    _MacroContext_prototype.isReturn = function (node) {
      node = this.real(node);
      return node instanceof LispyNode && node.isInternalCall("return");
    };
    _MacroContext_prototype.isAutoReturn = function (node) {
      node = this.real(node);
      return node instanceof LispyNode && node.isInternalCall("autoReturn");
    };
    _MacroContext_prototype.autoReturn = function (node) {
      return LispyNode.InternalCall("autoReturn", node.index, node.scope, node);
    };
    ...
    _MacroContext_prototype.isConst = function (node) {
      return node === void 0 || node instanceof LispyNode && this.real(node).isConst();
    };
    _MacroContext_prototype.value = function (node) {
      var expanded;
      if (node !== void 0 && node instanceof LispyNode) {
    ...
    _MacroContext_prototype.isNode = function (node) {
      return node instanceof LispyNode;
    };
    ...
    _MacroContext_prototype.isPrimordial = function (node) {
      node = this.real(node);
      return node instanceof Ident && node.isPrimordial();
    };
    ...
    _MacroContext_prototype.isCall = function (node) {
      node = this.real(node);
      return node instanceof LispyNode && node.isCall && !node.isInternalCall();
    };
    _MacroContext_prototype.isContextCall = function (node) {
      node = this.real(node);
      return node instanceof LispyNode && node.isInternalCall("contextCall");
    };
    ...
    _MacroContext_prototype.isNew = function (node) {
      node = this.real(node);
      return node instanceof LispyNode && node.isInternalCall("new");
    };
    _MacroContext_prototype["new"] = function (func) {
      var args;
      args = __slice.call(arguments, 1);
      return LispyNode.InternalCall.apply(LispyNode, ["new", this.index, this.scope(), func].concat(args));
    };
    ...
    _MacroContext_prototype.isSuper = function (node) {
      node = this.real(node);
      return node instanceof LispyNode && node.isInternalCall("super");
    };
    ...
    _MacroContext_prototype.call = function (func, args) {
      var _this;
      _this = this;
      if (args == null) {
        args = [];
      }
      return LispyNode.Call.apply(LispyNode, [func.index, this.scope(), func].concat((function () {
    ...
    _MacroContext_prototype.func = function (params, body, bound, asType, isGenerator) {
      var _arr, _i, _len, func, p, param, scope;
      if (bound == null) {
        bound = false;
      }
      if (asType == null) {
      ...
      for (_arr = [], _i = 0, _len = params.length; _i < _len; ++_i) {
        param = params[_i];
      ...
      params = _arr;
      func = LispyNode.InternalCall(
        "function",
        body.index,
        scope.parent,
        LispyNode.InternalCall.apply(LispyNode, ["array", body.index, scope.parent].concat(params)),
        body.rescope(scope),
        typeof bound === "boolean" ? LispyNode.Value(body.index, bound) : bound,
        asType || LispyNode.Symbol.nothing(body.index),
        LispyNode.Value(body.index, isGenerator)
      ).reduce(this.parser);
    ...
    _MacroContext_prototype.isFunc = function (node) {
      node = this.real(node);
      return node instanceof LispyNode && node.isInternalCall("function");
    };
    ...
    _MacroContext_prototype.param = function (ident, defaultValue, spread, isMutable, asType) {
      ...
      return LispyNode.InternalCall(
        ...
        ident.scope,
        ident,
        defaultValue || LispyNode.Symbol.nothing(ident.index),
        LispyNode.Value(ident.index, spread),
        LispyNode.Value(ident.index, isMutable),
        asType || LispyNode.Symbol.nothing(ident.index)
      ).reduce(this.parser);
    };
    _MacroContext_prototype.isParam = function (node) {
      node = this.real(node);
      return node instanceof LispyNode && node.isInternalCall("param");
    };
    ...
    _MacroContext_prototype.paramDefaultValue = function (node) {
      ...
      if (this.isParam(node)) {
        ...
        if (defaultValue instanceof LispyNode && defaultValue.isSymbol && defaultValue.isInternal && defaultValue.isNothing) {
          return null;
        } else {
          return defaultValue;
        }
    ...
    _MacroContext_prototype.paramType = function (node) {
      ...
      if (this.isParam(node)) {
        asType = node.args[4];
        if (asType instanceof LispyNode && asType.isSymbol && asType.isInternal && asType.isNothing) {
          return null;
        } else {
          return asType;
        }
    ...
    _MacroContext_prototype.pairs = function (node) {
      ...
      node = this.real(node);
      if (node instanceof LispyNode && node.isInternalCall("typeObject")) {
        for (_arr = [], i = 0, _end = +node.args.length; i < _end; i += 2) {
    ...
    _MacroContext_prototype.object = function (pairs, prototype) {
      ...
      for (_arr = [], i = 0, _len = pairs.length; i < _len; ++i) {
        ...
        if (pair instanceof LispyNode) {
          if (pair.isInternalCall("array")) {
            ...
            if (pair.args.length === 3 && (!(pair.args[2] instanceof LispyNode) || !pair.args[2].isConstType("string"))) {
          ...
          _arr.push(LispyNode.InternalCall.apply(LispyNode, [
            ...
            ? (typeof property === "string"
              ...
              : property instanceof LispyNode && property.isConstType("string") ? [property]
      ...
      arrayPairs = _arr;
      return LispyNode.InternalCall.apply(LispyNode, ["object", this.index, this.scope(), prototype || LispyNode.Symbol.nothing(this.index)].concat(arrayPairs)).reduce(this.parser);
    };
    _MacroContext_prototype.type = function (node) {
      if (typeof node === "string") {
        ...
        throw Error("Can only retrieve type from a String or LispyNode, got " + __typeof(node));
      }
    };
    ...
    _MacroContext_prototype.isNothing = function (node) {
      node = this.real(node);
      return node instanceof LispyNode && node.isSymbol && node.isInternal && node.isNothing;
    };
    ...
    _MacroContext_prototype.isTypeGeneric = function (node) {
      node = this.real(node);
      return node instanceof LispyNode && node.isInternalCall("typeGeneric");
    };
    ...
    _MacroContext_prototype.isTypeObject = function (node) {
      node = this.real(node);
      return node instanceof LispyNode && node.isInternalCall("typeObject");
    };
    _MacroContext_prototype.isTypeUnion = function (node) {
      node = this.real(node);
      return node instanceof LispyNode && node.isInternalCall("typeUnion");
    };
    ...
    function constifyObject(position, obj, index, scope) {
      ...
      if (obj === null || (_ref = typeof obj) === "string" || _ref === "number" || _ref === "boolean" || _ref === "undefined") {
      ...
      } else if (obj instanceof Ident && obj.name.length > 1 && obj.name.charCodeAt(0) === 36) {
        return LispyNode.Call(
          obj.index,
          scope,
      ...
      } else if (obj instanceof LispyNode.Call && obj.func instanceof Ident && obj.func.name === "$") {
        ...
        case "symbol":
          return LispyNode.Call.apply(LispyNode, [
            obj.index,
            scope,
        ...
        case "macroAccess":
          return LispyNode.Call(
            ...
            LispyNode.Value(obj.index, obj.inStatement),
            LispyNode.Value(obj.index, obj.inGenerator),
            LispyNode.Value(obj.index, obj.inEvilAst),
            LispyNode.Value(obj.index, obj.doWrapped)
          );
        default: throw Error("Unhandled value in switch");
        }
      } else if (obj.constructor === Object) {
        return LispyNode.InternalCall.apply(LispyNode, ["object", index, scope, LispyNode.Symbol.nothing(index)].concat((function () {
          var _arr, k, v;
          _arr = [];
          for (k in obj) {
    ...
    _MacroContext_prototype.makeLispyCall = function (fromPosition, func) {
      ...
      return LispyNode.Call.apply(LispyNode, [index, this.scope(), func].concat(args));
    };
    ...
    function toLiteralNode(obj) {
      ...
      if (obj === null || (_ref = typeof obj) === "undefined" || _ref === "boolean" || _ref === "number" || _ref === "string") {
        ...
        return LispyNode.InternalCall.apply(LispyNode, ["object", 0, this.scope(), LispyNode.Symbol.nothing(0)].concat((function () {
          var _arr, k, v;
          _arr = [];
          for (k in obj) {
            if (__owns.call(obj, k)) {
    ...
    _MacroContext_prototype.hasFunc = function (node) {
      ...
      if (this._hasFunc != null) {
        ...
        FOUND = {};
        walker = function (x) {
          if (x instanceof LispyNode && x.isInternalCall("function")) {
            throw FOUND;
          }
        };
        try {
    ...
    _MacroContext_prototype.isStatement = function (node) {
      node = this.macroExpand1(node);
      return node instanceof LispyNode && node.isStatement();
    };
