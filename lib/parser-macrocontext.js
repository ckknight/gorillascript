(function () {
  ...
  var __create, __isArray, __owns, __slice, __throw, __toArray, __typeof, _ref,
      ...
      BinaryNode, BlockNode, CallNode, CommentNode, ContinueNode, DebuggerNode,
      DefNode, EmbedWriteNode, EvalNode, ForInNode, ForNode, FunctionNode,
      IdentNode, IfNode, LispyNode, MacroAccessNode, MacroConstNode,
      MacroContext, Node, nodeToType, NothingNode, ObjectNode, ParamNode,
      RegexpNode, ReturnNode, RootNode, Scope, SpreadNode, SuperNode,
      SwitchNode, SyntaxChoiceNode, SyntaxManyNode, SyntaxParamNode,
      SyntaxSequenceNode, ThrowNode, TmpNode, TmpWrapperNode, TryCatchNode,
  ...
  MacroContext = (function () {
    ...
    _MacroContext_prototype.label = function (node) {
      ...
      if (node instanceof LispyNode) {
        if (node.isCall && node.func.isBreak) {
          return node.args[0];
        } else {
          return null;
        }
      } else if (node instanceof ContinueNode || node instanceof BlockNode || node instanceof IfNode || node instanceof SwitchNode || node instanceof ForNode || node instanceof ForInNode || node instanceof TryCatchNode || node instanceof TryFinallyNode) {
        return node.label;
      } else {
        return null;
      }
