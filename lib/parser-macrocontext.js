(function () {
  ...
  var __create, __isArray, __owns, __slice, __throw, __toArray, __typeof, _ref,
      AccessNode, addParamToScope, AssignNode, BinaryNode, CallNode,
      EmbedWriteNode, FunctionNode, IdentNode, LispyNode, MacroAccessNode,
      MacroConstNode, MacroContext, Node, nodeToType, NothingNode, ParamNode,
      RootNode, Scope, SuperNode, SyntaxChoiceNode, SyntaxManyNode,
      SyntaxParamNode, SyntaxSequenceNode, TmpNode, Type, TypeFunctionNode,
      TypeGenericNode, TypeObjectNode, TypeUnionNode, UnaryNode;
  ...
  MacroContext = (function () {
    ...
    _MacroContext_prototype.block = function (nodes, label) {
      ...
      return LispyNode.InternalCall.apply(LispyNode, [
        ...
        this.index,
        this.scope(),
        label || NothingNode(this.index, this.scope())
      ].concat(__toArray(nodes))).reduce(this.parser);
    };
    _MacroContext_prototype["if"] = function (test, whenTrue, whenFalse, label) {
      ...
      return LispyNode.InternalCall.apply(LispyNode, [
        ...
        whenTrue,
        whenFalse
      ].concat(label ? [label] : [])).reduce(this.parser);
    };
    _MacroContext_prototype["switch"] = function (topic, cases, defaultCase, label) {
    ...
    _MacroContext_prototype.real = function (node) {
      ...
      if (node instanceof LispyNode && node.isCall && node.func.isSymbol && node.func.isInternal && node.func.isTmpWrapper) {
        return node.args[0];
      } else {
        return node;
      }
    ...
    _MacroContext_prototype.rewrap = function (newNode, oldNode) {
      ...
      if (oldNode instanceof LispyNode && oldNode.isCall && oldNode.func.isSymbol && oldNode.func.isInternal && oldNode.func.isTmpWrapper) {
        if (newNode instanceof LispyNode && newNode.isCall && newNode.func.isSymbol && newNode.func.isInternal && newNode.func.isTmpWrapper) {
    ...
    _MacroContext_prototype.isLabeledBlock = function (node) {
      ...
      if (node instanceof LispyNode && node.isCall) {
        ...
        if (func.isSymbol && func.isInternal) {
          switch (func.name) {
          case "block": return !(node.args[0] instanceof NothingNode);
          case "tryFinally": return node.args[2] != null;
          case "tryCatch":
          case "forIn":
          case "if": return node.args[3] != null;
          case "for": return node.args[4] != null;
          case "switch":
            args = node.args;
            if (args.length % 3 === 0) {
    ...
    _MacroContext_prototype.isBreak = function (node) {
      node = this.real(node);
      return node instanceof LispyNode && node.isCall && node.func.isBreak;
    };
    _MacroContext_prototype.isContinue = function (node) {
      node = this.real(node);
      return node instanceof LispyNode && node.isCall && node.func.isContinue;
    };
    _MacroContext_prototype.label = function (node) {
      ...
      if (node instanceof LispyNode && node.isCall) {
        ...
        if (func.isSymbol && func.isInternal) {
          switch (func.name) {
          case "break":
          case "continue": return node.args[0];
          case "block":
            if (!(node.args[0] instanceof NothingNode)) {
              return node.args[0];
            }
            break;
          case "tryFinally": return node.args[2];
          case "tryCatch":
          case "forIn":
          case "if": return node.args[3];
          case "for": return node.args[4];
          case "switch":
            args = node.args;
            if (args.length % 3 === 0) {
    ...
    _MacroContext_prototype.isSpread = function (node) {
      node = this.real(node);
      return node instanceof LispyNode && node.isCall && node.func.isSymbol && node.func.isInternal && node.func.isSpread;
    };
    ...
    _MacroContext_prototype.isArray = function (node) {
      node = this.real(node);
      return node instanceof LispyNode && node.isCall && node.func.isSymbol && node.func.isInternal && node.func.isArray;
    };
    ...
    _MacroContext_prototype.isObject = function (node) {
      node = this.real(node);
      return node instanceof LispyNode && node.isCall && node.func.isSymbol && node.func.isInternal && node.func.isObject;
    };
    _MacroContext_prototype.pairs = function (node) {
      ...
      if (this.isTypeObject(node)) {
        return node.pairs;
      } else if (node instanceof LispyNode && node.isCall && node.func.isSymbol && node.func.isInternal && node.func.isObject) {
    ...
    _MacroContext_prototype.isBlock = function (node) {
      node = this.real(node);
      return node instanceof LispyNode && node.isCall && node.func.isSymbol && node.func.isInternal && node.func.isBlock;
    };
    ...
    _MacroContext_prototype.object = function (pairs, prototype) {
      ...
      for (_arr = [], i = 0, _len = pairs.length; i < _len; ++i) {
        ...
        if (pair instanceof OldNode) {
          if (pair instanceof LispyNode && pair.isCall && pair.func.isSymbol && pair.func.isInternal && pair.func.isArray) {
    ...
    _MacroContext_prototype.isCustom = function (node) {
      node = this.real(node);
      return node instanceof LispyNode && node.isCall && node.func.isSymbol && node.func.isInternal && node.func.isCustom;
    };
    ...
    _MacroContext_prototype.isIf = function (node) {
      node = this.real(node);
      return node instanceof LispyNode && node.isCall && node.func.isSymbol && node.func.isInternal && node.func.isIf;
    };
    _MacroContext_prototype.test = function (node) {
      node = this.real(node);
      return node instanceof LispyNode && node.isCall && node.func.isSymbol && node.func.isInternal && node.func.isIf && node.args[0];
    };
    _MacroContext_prototype.whenTrue = function (node) {
      node = this.real(node);
      return node instanceof LispyNode && node.isCall && node.func.isSymbol && node.func.isInternal && node.func.isIf && node.args[1];
    };
    _MacroContext_prototype.whenFalse = function (node) {
      node = this.real(node);
      return node instanceof LispyNode && node.isCall && node.func.isSymbol && node.func.isInternal && node.func.isIf && node.args[2];
    };
    ...
    _MacroContext_prototype.maybeCache = function (node, func, name, save) {
      ...
      if (this.isComplex(node)) {
        ...
        setTmp = LispyNode.InternalCall(
          ...
          this.index,
          this.scope(),
          NothingNode(this.index, this.scope()),
    ...
    _MacroContext_prototype.empty = function (node) {
      ...
      if (node == null) {
        ...
        return false;
      } else if (node instanceof LispyNode && node.isCall && node.func.isSymbol && node.func.isInternal && node.func.isBlock) {
        _every = true;
        for (_arr = __toArray(node.args), _i = _arr.length - 1; _i >= 1; --_i) {
          item = _arr[_i];
          if (!this.empty(item)) {
            _every = false;
            break;
    ...
    function constifyObject(position, obj, index, scope) {
      ...
      if (obj === null || (_ref = typeof obj) === "string" || _ref === "number" || _ref === "boolean" || _ref === "undefined") {
        ...
        arg = obj.args[0];
        if (arg instanceof LispyNode && arg.isCall && arg.func.isSymbol && arg.func.isInternal && arg.func.isSpread) {
    ...
    _MacroContext_prototype.wrap = function (value) {
      ...
      if (__isArray(value)) {
        return LispyNode.InternalCall.apply(LispyNode, [
          ...
          this.scope(),
          NothingNode(this.index, this.scope())
        ].concat(__toArray(value))).reduce(this.parser);
    ...
    function walk(node, func) {
      ...
      if ((!(node instanceof LispyNode) || !node.isCall || !node.func.isSymbol || !node.func.isInternal || !node.func.isBlock) && (_ref = func.call(this, node)) != null) {
        return _ref;
      }
      return node.walk(
        function (x) {
