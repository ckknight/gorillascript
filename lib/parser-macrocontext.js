(function () {
  ...
  var __create, __isArray, __owns, __slice, __throw, __toArray, __typeof, _ref,
      ...
      IdentNode, IfNode, LispyNode, MacroAccessNode, MacroConstNode,
      MacroContext, Node, nodeToType, NothingNode, ObjectNode, ParamNode,
      RootNode, Scope, SpreadNode, SuperNode, SwitchNode, SyntaxChoiceNode,
      SyntaxManyNode, SyntaxParamNode, SyntaxSequenceNode, TmpNode,
      TmpWrapperNode, Type, TypeFunctionNode, TypeGenericNode, TypeObjectNode,
      TypeUnionNode, UnaryNode, VarNode;
  ...
  MacroContext = (function () {
    ...
    _MacroContext_prototype.isLabeledBlock = function (node) {
      node = this.real(node);
      if (node instanceof BlockNode || node instanceof IfNode || node instanceof SwitchNode) {
        ...
        } else if (node.func.isTryCatch || node.func.isForIn) {
        ...
        if (node.isCall) {
          ...
          if (func.isSymbol && func.isInternal) {
            if (func.isBreak || func.isContinue) {
              ...
              return node.args[2];
            } else if (func.isTryCatch || func.isForIn) {
              return node.args[3];
            } else if (func.isTryCatch || func.isFor) {
              return node.args[4];
            }
          }
        }
      } else if (node instanceof BlockNode || node instanceof IfNode || node instanceof SwitchNode) {
        return node.label;
      }
      return null;
    };
