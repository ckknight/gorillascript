(function () {
  ...
  var __create, __isArray, __owns, __slice, __throw, __toArray, __typeof, _ref,
      ...
      DefNode, EmbedWriteNode, FunctionNode, IdentNode, LispyNode,
      ...
      SyntaxSequenceNode, TmpNode, TmpWrapperNode, Type, TypeFunctionNode,
  ...
  MacroContext = (function () {
    ...
    _MacroContext_prototype.isLabeledBlock = function (node) {
      node = this.real(node);
      if (node instanceof BlockNode || node instanceof SwitchNode) {
        return node.label != null;
      } else if (node instanceof LispyNode && node.isCall && node.func.isSymbol && node.func.isInternal) {
        if (node.func.isTryFinally) {
          return node.args[2] != null;
        } else if (node.func.isTryCatch || node.func.isForIn || node.func.isIf) {
          return node.args[3] != null;
        } else if (node.func.isFor) {
          return node.args[4] != null;
        } else {
          return false;
        }
      } else {
    ...
    _MacroContext_prototype.label = function (node) {
      ...
      if (node instanceof LispyNode) {
        if (node.isCall) {
          ...
          if (func.isSymbol && func.isInternal) {
            if (func.isBreak || func.isContinue) {
              return node.args[0];
            } else if (func.isTryFinally) {
              return node.args[2];
            } else if (func.isTryCatch || func.isForIn || func.isIf) {
              return node.args[3];
            } else if (func.isFor) {
              return node.args[4];
            }
      ...
      } else if (node instanceof BlockNode || node instanceof SwitchNode) {
        return node.label;
      }
      return null;
    };
