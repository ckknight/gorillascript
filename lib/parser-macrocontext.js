(function () {
  ...
  var __create, __isArray, __owns, __slice, __throw, __toArray, __typeof, _ref,
      addParamToScope, Cache, Ident, LispyNode, MacroContext, nodeToType, Scope,
      Tmp, Type;
  ...
  MacroContext = (function () {
    ...
    _MacroContext_prototype.rewrap = function (newNode, oldNode) {
      if (newNode instanceof LispyNode) {
        if (!(oldNode instanceof LispyNode)) {
          throw new TypeError("Expected oldNode to be a LispyNode, got " + __typeof(oldNode));
        }
    ...
    _MacroContext_prototype.eq = function (alpha, bravo) {
      ...
      return alpha === bravo || alpha instanceof LispyNode && alpha.equals(bravo);
    };
    ...
    _MacroContext_prototype.type = function (node) {
      if (typeof node === "string") {
        ...
        throw new Error("Can only retrieve type from a String or LispyNode, got " + __typeof(node));
      }
    };
    ...
    _MacroContext_prototype.op = function (node) {
      node = this.real(node);
      if (node instanceof LispyNode && node.isCall && node.func.isSymbol && node.func.isOperator) {
        return node.func.name;
      }
    };
    _MacroContext_prototype.left = function (node) {
      node = this.real(node);
      if (node instanceof LispyNode && (node.isBinaryCall() || node.isAssignCall())) {
        return node.args[0];
      }
    };
    ...
    _MacroContext_prototype.unaryNode = function (node) {
      node = this.real(node);
      if (node instanceof LispyNode && node.isUnaryCall()) {
        return node.args[0];
      }
    };
