(function () {
  ...
  var __create, __isArray, __owns, __slice, __throw, __toArray, __typeof, _ref,
      ...
      BinaryNode, BlockNode, CallNode, CommentNode, DefNode, EmbedWriteNode,
      EvalNode, ForInNode, ForNode, FunctionNode, IdentNode, IfNode, LispyNode,
      MacroAccessNode, MacroConstNode, MacroContext, Node, nodeToType,
      NothingNode, ObjectNode, ParamNode, RegexpNode, RootNode, Scope,
      SpreadNode, SuperNode, SwitchNode, SyntaxChoiceNode, SyntaxManyNode,
      SyntaxParamNode, SyntaxSequenceNode, TmpNode, TmpWrapperNode,
  ...
  MacroContext = (function () {
    ...
    _MacroContext_prototype["throw"] = function (node) {
      ...
      return LispyNode.Call(this.index, this.scope(), LispyNode.Symbol["throw"](this.index), this.doWrap(node)).reduce(this.parser);
    };
    _MacroContext_prototype["return"] = function (node) {
      if (node == null) {
        node = NothingNode(0, this.scope());
      }
      return LispyNode.Call(this.index, this.scope(), LispyNode.Symbol["return"](this.index), this.doWrap(node)).reduce(this.parser);
    };
    _MacroContext_prototype["yield"] = function (node) {
      if (node == null) {
        node = NothingNode(0, this.scope());
      }
      return LispyNode.Call(this.index, this.scope(), LispyNode.Symbol["yield"](this.index), this.doWrap(node)).reduce(this.parser);
    };
    _MacroContext_prototype["debugger"] = function () {
      return LispyNode.Call(this.index, this.scope(), LispyNode.Symbol["debugger"](this.index));
    };
    _MacroContext_prototype["break"] = function (label) {
      if (label == null) {
        label = null;
      }
      return LispyNode.Call.apply(LispyNode, [this.index, this.scope(), LispyNode.Symbol["break"](this.index)].concat(label ? [label] : []));
    };
    _MacroContext_prototype["continue"] = function (label) {
      if (label == null) {
        label = null;
      }
      return LispyNode.Call.apply(LispyNode, [this.index, this.scope(), LispyNode.Symbol["continue"](this.index)].concat(label ? [label] : []));
    };
    ...
    _MacroContext_prototype.label = function (node) {
      ...
      } else if (node instanceof BlockNode || node instanceof IfNode || node instanceof SwitchNode || node instanceof ForNode || node instanceof ForInNode || node instanceof TryCatchNode || node instanceof TryFinallyNode) {
        return node.label;
      } else {
        return null;
      }
