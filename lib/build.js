(function () {
  "use strict";
  var __async, __create, __in, __isArray, __num, __once, __slice, __strnum, __toArray, __typeof, fs, gorilla, path, startTime;
  ...
  fs.readdir("./src", function (err, files) {
    ...
    argv = __slice.call(process.argv, 2);
    options = {};
    if (argv[0] === "--uglify") {
      options.uglify = true;
    ...
    files = (function () {
      ...
      for (_arr = [], _arr2 = __toArray(files.sort()), _i = 0, _len = _arr2.length; _i < _len; ++_i) {
        file = _arr2[_i];
        if ((argv.length === 0 || __in(file, argv)) && file.match(/\.gs$/i) && file !== "prelude.gs") {
          _arr.push(file);
        }
      }
      return _arr;
    ...
    return gorilla.init(function (_e, err) {
      ...
      return __async(
        ...
        function (err) {
          var _arr, _i, _len, _ref, current, file, longestNameLen, results, totals, totalTime;
          if (err != null) {
            return done(err);
          }
          current = 0;
          ...
          write("     parse     macro     reduce    translate compile " + (options.uglify ? "  uglify  " : "") + "|  total\n");
          totals = {};
          totalTime = 0;
          results = {};
          return __async(
            ...
            __num(files.length),
            function (_i, next) {
              var _o, _ref, code, file, filename, startFileTime;
              file = files[_i];
              ...
              return gorilla.compile(
                code,
                (_o = __create(options), _o.filename = filename, _o.progress = progress, _o),
                function (err, compiled) {
                  var endFileTime, fileTime;
                  if (err != null) {
                    write("\n");
                    return next(err);
                  }
                  results[file] = compiled.code;
                  endFileTime = Date.now();
                  fileTime = endFileTime - startFileTime;
                  write(" | " + padLeft((fileTime / 1000).toFixed(3), 6, " ") + " s\n");
                  totalTime += fileTime;
            ...
            function (err) {
              ...
              if (__num(files.length) > 1) {
                ...
                for (_arr = [
                  ...
                  "translate",
                  "compile"
                ].concat(options.uglify ? ["uglify"] : []), _i = 0, _len = _arr.length; _i < _len; ++_i) {
                  part = _arr[_i];
