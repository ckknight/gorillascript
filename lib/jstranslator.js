(function (GLOBAL) {
  ...
  generatorTranslate = (function () {
    var expressions, generatorTranslateExpressionLispyInternals,
    ...
    expressions = {
      9: function (node, scope, state, assignTo, unassigned) {
        var args, gArgs, gChild, gCode, gFunc, gParent, isNew;
        if (node.func instanceof LispyNode.Symbol.ident && node.func.name === "eval") {
        ...
        if (node.func instanceof LispyNode && node.func.isInternalCall("access")) {
  ...
  translators = {
    9: function (node, scope, location, unassigned) {
      var args, isNew, tArgArray, tCode, tFunc;
      if (node.func instanceof LispyNode.Symbol.ident) {
        if (node.func.name === "RegExp" && node.args[0].isConst() && (!node.args[1] || node.args[1].isConst())) {
          if (node.args[1] && node.args[1].constValue()) {
            return function () {
              return ast.Regex(getPos(node), String(node.args[0].constValue()), String(node.args[1].constValue()));
            };
          } else {
            return function () {
              return ast.Regex(getPos(node), String(node.args[0].constValue()));
            };
          }
        } else if (node.func.name === "eval") {
          tCode = translate(node.args[0], scope, "expression", unassigned);
          return function () {
      ...
      return function () {
        ...
        if (argArray instanceof ast.Arr) {
          ...
          return scope.maybeCache(func.left, Type["function"], (_once = false, function (setParent, parent) {
            if (_once) {
              throw Error("Attempted to call function more than once");
            } else {
              _once = true;
    ...
    20: (function () {
      ...
      translateTypeChecks = {
        ...
        46: function (node) {
          ...
          for (_arr = __toArray(node.pairs), _i = 0, _len = _arr.length; _i < _len; ++_i) {
            ...
            value = _ref.value;
            if (key.isConst()) {
              typeData[key.constValue()] = translateTypeCheck(value);
            }
          }
      ...
      translateType = (function () {
        var primordialTypes, translateTypes;
        translateTypes = {
