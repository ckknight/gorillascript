(function (GLOBAL) {
  ...
  generatorTranslate = (function () {
    ...
    expressions = {
      9: function (node, scope, state, assignTo, unassigned) {
        ...
        if (node.func instanceof ParserNode.Ident && node.func.name === "eval") {
    ...
    generatorTranslateExpressionLispyOperators = {
      assign: function (node, args, scope, state, assignTo, unassigned) {
        ...
        if (left instanceof LispyNode && left.isInternalCall("access")) {
          ...
          if (unassigned && left instanceof ParserNode.Ident) {
    ...
    generatorTranslateLispyInternals = {
      ...
      "for": function (node, args, scope, state, _p, _p2, unassigned) {
        var bodyBranch, bodyUnassigned, gTest, k, postBranch, stepBranch,
            testBranch, v;
        if (args[0] != null && !(args[0] instanceof ParserNode.Nothing)) {
          state = generatorTranslate(
            args[0],
            scope,
            state,
        ...
        stepBranch = null;
        if (args[2] != null && !(args[2] instanceof ParserNode.Nothing)) {
      ...
      forIn: function (node, args, scope, state, _p, _p2, unassigned) {
        ...
        if (unassigned && args[0] instanceof ParserNode.Ident) {
      ...
      "if": function (node, args, scope, state, breakState, continueState, unassigned) {
        ...
        if (state.hasGeneratorNode(args[1]) || state.hasGeneratorNode(args[2])) {
          ...
          if (!(args[1] instanceof ParserNode.Nothing)) {
            whenTrueBranch = state.branch();
          }
          if (whenTrueBranch) {
            generatorTranslate(
          ...
          if (!(args[2] instanceof ParserNode.Nothing)) {
            whenFalseBranch = state.branch();
          }
          if (whenFalseBranch) {
            generatorTranslate(
      ...
      "switch": function (node, args, scope, state, _p, continueState, unassigned) {
        ...
        if (!(data.defaultCase instanceof ParserNode.Nothing)) {
  ...
  translateLispyInternal = {
    ...
    object: function (node, args, scope, location, unassigned) {
      ...
      if (!(args[0] instanceof ParserNode.Nothing)) {
    ...
    "var": function (node, args, scope, location, unassigned) {
      var ident, tIdent;
      ident = args[0];
      if (unassigned && !unassigned["\u0000"] && ident instanceof ParserNode.Ident && !__owns.call(unassigned, ident.name)) {
  ...
  translateLispyOperator = {
    ...
    unary: function (node, args, scope, location, unassigned) {
      var opName, tSubnode;
      opName = node.func.name;
      if (unassigned && (opName === "++" || opName === "--" || opName === "++post" || opName === "--post") && args[0] instanceof ParserNode.Ident) {
        unassigned[args[0].name] = false;
      }
