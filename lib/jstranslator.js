(function (GLOBAL) {
  ...
  generatorTranslate = (function () {
    ...
    expressions = {
      9: function (node, scope, state, assignTo, unassigned) {
        var args, gArgs, gChild, gCode, gFunc, gParent, gStart, isApply, isNew;
        ...
        isNew = node.isNew;
        args = node.args;
        if (isApply && (args.length === 0 || !(args[0] instanceof LispyNode) || !args[0].isInternalCall("spread"))) {
          if (args.length === 0) {
            gStart = {
              state: gFunc.state,
              tNode: function () {
  ...
  translators = {
    9: function (node, scope, location, unassigned) {
      var args, isApply, isNew, tArgArray, tCode, tFunc, tStart;
