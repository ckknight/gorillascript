(function (GLOBAL) {
  ...
  var __arrayToIter, __cmp, __create, __curry, __first, __import,
      ...
      MacroHolder, Map, ParserNode, Scope, translateLispyInternal,
      translateLispyOperator, translators, Type;
  ...
  function isNothing(node) {
    return node instanceof LispyNode.Symbol.nothing;
  }
  Scope = (function () {
  ...
  function makeHasGeneratorNode() {
    ...
    function hasInLoop(node) {
      ...
      return inLoopCache.getOrAdd(node, (_once = false, function (node) {
        ...
        result = false;
        if (node instanceof LispyNode && node.isInternalCall("yield", "return")) {
          return true;
        }
    ...
    function hasInSwitch(node) {
      ...
      return inSwitchCache.getOrAdd(node, (_once = false, function (node) {
        ...
        if (node instanceof LispyNode && node.isInternalCall("continue", "yield", "return")) {
          return true;
        }
        if (!(node instanceof ParserNode.Function)) {
          FOUND = {};
          try {
            node.walk(function (n) {
              if (n instanceof LispyNode && n.isInternalCall("for", "forIn")) {
                if (hasInLoop(n)) {
                  throw FOUND;
                }
    ...
    function hasGeneratorNode(node, allowReturn) {
      ...
      return (allowReturn ? returnFreeCache : normalCache).getOrAdd(node, (_once = false, function (node) {
        ...
        if (node instanceof LispyNode && (node.isInternalCall("break", "continue", "yield") || !allowReturn && node.isInternalCall("return"))) {
          return true;
        }
        if (!(node instanceof ParserNode.Function)) {
          FOUND = {};
          try {
            node.walk(function (n) {
              if (n instanceof LispyNode && n.isInternalCall()) {
                switch (n.func.name) {
                case "forIn":
                case "for":
                  if (hasInLoop(n)) {
              ...
              if (hasGeneratorNode(n, allowReturn)) {
                throw FOUND;
              }
              return n;
            });
  ...
  function flattenSpreadArray(elements) {
    ...
    for (_arr = __toArray(elements), _i = 0, _len = _arr.length; _i < _len; ++_i) {
      element = _arr[_i];
      if (element instanceof LispyNode && element.isInternalCall("spread")) {
        node = element.args[0];
  ...
  generatorTranslate = (function () {
    var generatorTranslateExpressionLispyInternals,
    ...
    function hasSingleNodeWithNoopsNoSpread(nodes, state) {
      ...
      for (_i = 0, _len = nodes.length; _i < _len; ++_i) {
        node = nodes[_i];
        if (node instanceof LispyNode && node.isInternalCall("spread")) {
          return false;
    ...
    generatorTranslateExpressionLispyInternals = {
      access: function (node, args, scope, state, assignTo, unassigned) {
        var gChild, gParent;
      ...
      contextCall: function (node, args, scope, state, assignTo, unassigned) {
        ...
        if (!(context instanceof LispyNode) || !context.isInternalCall("spread")) {
      ...
      "new": function (node, args, scope, state, assignTo, unassigned) {
        var gArgs, gFunc;
        gFunc = generatorTranslateExpression(
          args[0],
          scope,
      ...
      "yield": function (node, args, scope, state, assignTo, unassigned) {
        var gNode;
      ...
      "if": function (node, args, scope, state, assignTo, unassigned) {
      ...
      array: function (node, args, scope, state, assignTo, unassigned) {
      ...
      tmpWrapper: function (node, args, scope, state, assignTo, unassigned) {
        var gNode;
        gNode = generatorTranslateExpression(
          args[0],
          scope,
      ...
      block: function (node, args, scope, state, assignTo, unassigned) {
    ...
    generatorTranslateExpressionLispyOperators = {
      assign: function (node, args, scope, state, assignTo, unassigned) {
        var gChild, gLeft, gParent, gRight, left, right;
        left = args[0];
        right = args[1];
        if (left instanceof LispyNode && left.isInternalCall("access")) {
          gParent = generatorTranslateExpression(
            left.args[0],
            scope,
            state,
          ...
          if (unassigned && left instanceof LispyNode.Symbol.ident) {
      ...
      unary: function (node, args, scope, state, assignTo, unassigned) {
        var gNode;
        gNode = generatorTranslateExpression(
          args[0],
          scope,
        ...
        return handleAssign(assignTo, scope, gNode.state, function () {
          return ast.Unary(getPos(node), node.func.name, __first(gNode.tNode(), gNode.cleanup()));
        });
      }
    };
    function generatorTranslateExpressionLispyCall(node, func, args, scope, state, assignTo, unassigned) {
      ...
      if (func instanceof LispyNode.Symbol.ident && func.name === "eval") {
    ...
    function generatorTranslateExpressionLispy(node, scope, state, assignTo, unassigned) {
      ...
      if (node.isCall) {
        ...
        if (func.isSymbol) {
          if (func.isInternal) {
            name = func.name;
            if (!__owns.call(generatorTranslateExpressionLispyInternals, name)) {
              throw Error("Unable to translate internal call for '" + name + "'");
            }
            return generatorTranslateExpressionLispyInternals[name](
              node,
              args,
              scope,
              state,
            ...
            name = func.operatorType;
            if (!__owns.call(generatorTranslateExpressionLispyOperators, name)) {
              throw Error("Unable to translate operator call for '" + name + "'");
            }
            return generatorTranslateExpressionLispyOperators[name](
              node,
              args,
              scope,
              state,
    ...
    function generatorTranslateExpression(node, scope, state, assignTo, unassigned) {
      ...
      if (state.hasGeneratorNode(node)) {
        ...
        throw Error("Unable to translate non-LispyNode: " + __typeof(node));
      } else {
    ...
    function isExpression(node) {
      if (node instanceof LispyNode) {
        if (node.isInternalCall()) {
          return __owns.call(generatorTranslateExpressionLispyInternals, node.func.name);
        } else {
          return true;
        }
      } else {
    ...
    generatorTranslateLispyInternals = {
      block: function (node, args, scope, state, breakState, continueState, unassigned, isTop) {
      ...
      "break": function (node, args, scope, state, breakState) {
        if (args[0]) {
      ...
      "continue": function (node, args, scope, state, breakState, continueState) {
        if (args[0]) {
      ...
      embedWrite: function (node, args, scope, state, breakState, continueState, unassigned) {
        var gText;
        ...
        return gText.state.add(function () {
          return ast.Call(
            ...
            [__first(gText.tNode(), gText.cleanup())].concat(args[1].constValue()
              ? [ast.Const(getPos(node), true)]
              : [])
          );
        });
      },
      "throw": function (node, args, scope, state) {
        var gNode;
      ...
      "yield": function (node, args, scope, state) {
        var gNode;
        gNode = generatorTranslateExpression(args[0], scope, state, false);
      ...
      "return": function (node, args, scope, state, breakState, continueState, unassigned, isTop) {
        var gNode, mutatedNode;
        mutatedNode = args[0].mutateLast(
          null,
          function (n) {
            if (n instanceof LispyNode && n.isInternalCall("return")) {
              return n;
            } else {
        ...
        if (mutatedNode instanceof LispyNode && mutatedNode.isInternalCall("return") && mutatedNode.args[0] === args[0]) {
      ...
      tryCatch: function (node, args, scope, state, breakState, continueState, unassigned, isTop) {
        var postBranch;
      ...
      tryFinally: function (node, args, scope, state, breakState, continueState, unassigned, isTop) {
        var tFinally;
      ...
      "for": function (node, args, scope, state, _p, _p2, unassigned) {
      ...
      forIn: function (node, args, scope, state, _p, _p2, unassigned) {
        ...
        if (unassigned && args[0] instanceof LispyNode.Symbol.ident) {
      ...
      "if": function (node, args, scope, state, breakState, continueState, unassigned) {
      ...
      "switch": function (node, args, scope, state, _p, continueState, unassigned) {
        ...
        for (_arr = data.cases, i = 0, _len = _arr.length, _f = function (case_, i) {
          ...
          caseBranch = gTopic.state.branch();
      ...
      tmpWrapper: function (node, args, scope, state, breakState, continueState, unassigned, isTop) {
    ...
    function generatorTranslateLispy(node, scope, state, breakState, continueState, unassigned, isTop) {
      ...
      if (node.isInternalCall()) {
        name = node.func.name;
        if (__owns.call(generatorTranslateLispyInternals, name)) {
          return generatorTranslateLispyInternals[name](
            node,
            node.args,
            scope,
            state,
  ...
  translators = {
    20: (function () {
      ...
      function translateTypeCheck(node) {
        ...
        if (node instanceof LispyNode) {
          if (node.isInternalCall("access")) {
            return Type.any;
          } else if (node.isSymbol && node.isIdent) {
            if (__owns.call(primitiveTypes, node.name)) {
              return Type[primitiveTypes[node.name]];
            } else {
              return Type.any;
            }
          } else if (node.isSymbol && node.isInternal && node.isNothing) {
            return Type.any;
          } else if (node.isInternalCall("typeUnion")) {
            result = Type.none;
            for (_arr = __toArray(node.types), _i = 0, _len = _arr.length, _f = function (type) {
              return result = result.union(type.isConst()
                ? (function () {
                  ...
                  default: throw Error("Unknown const value for typechecking: " + String(type.value));
                  }
                }())
                : type instanceof LispyNode.Symbol.ident
                ? (__owns.call(primitiveTypes, type.name) ? Type[primitiveTypes[type.name]] : Type.any)
                : __throw(Error("Not implemented: typechecking for non-idents/consts within a type-union")));
            }; _i < _len; ++_i) {
              _f.call(this, _arr[_i]);
            }
            return result;
          } else if (node.isInternalCall("typeGeneric")) {
            if (node.args[0].isIdent && node.args[0].name === "Array") {
              return translateTypeCheck(node.args[1]).array();
            } else if (node.args[0].isIdent && node.args[0].name === "Function") {
              return translateTypeCheck(node.args[1])["function"]();
            } else {
              return Type.any;
            }
          } else if (node.isInternalCall("typeObject")) {
            typeData = {};
      ...
      function translateParam(param, scope, inner) {
        ...
        if (!(param instanceof LispyNode) || !param.isInternalCall("param")) {
      ...
      return function (node, scope, location) {
        return function () {
          var _arr, _ref, body, convertAutoReturn, fakeThis, i, initializers,
              innerScope, len, p, param, paramIdents, realInnerScope,
              unassigned, wrap;
          ...
          if (node.bound instanceof ParserNode) {
            convertAutoReturn = function (node) {
              return node.args[0];
            };
          } else {
            convertAutoReturn = function (node) {
              return LispyNode.Call(node.index, node.scope, LispyNode.Symbol["return"](node.index), node.args[0]);
            };
          }
          function translateAutoReturn(node) {
            if (node instanceof ParserNode.Function) {
              return node;
            }
            if (node instanceof LispyNode && node.isInternalCall("autoReturn")) {
              node = convertAutoReturn(node);
            }
            return node.walk(translateAutoReturn);
          }
          unassigned = {};
          _ref = translateFunctionBody(
            ...
            node.generator,
            innerScope,
            translateAutoReturn(node.body),
            unassigned
          );
          body = _ref.body;
          wrap = _ref.wrap;
          innerScope.releaseTmps();
          body = ast.Block(getPos(node.body), initializers.concat([body]));
  ...
  translateLispyInternal = {
    access: function (node, args, scope, location, unassigned) {
      var tChild, tParent;
    ...
    contextCall: function (node, args, scope, location, unassigned) {
    ...
    "new": function (node, args, scope, location, unassigned) {
      var tArgs, tFunc;
    ...
    label: function (node, args, scope, location, unassigned) {
      var tLabel, tNode;
    ...
    block: function (node, args, scope, location, unassigned) {
      var _arr, _arr2, i, len, subnode, tNodes;
      for (_arr = [], _arr2 = __toArray(args), i = 0, len = _arr2.length; i < len; ++i) {
        subnode = _arr2[i];
    ...
    "break": function (node, args, scope) {
      var tLabel;
    ...
    "continue": function (node, args, scope) {
      var tLabel;
      tLabel = args[0] && translate(args[0], scope, "label");
      return function () {
    ...
    embedWrite: function (node, args, scope, location, unassigned) {
      var innerScope, tText, wrapped;
      if (args[0].isStatement()) {
        innerScope = args[0].scope.clone();
        wrapped = LispyNode.Call(args[0].index, args[0].scope, ParserNode.Function(
          args[0].index,
          innerScope,
          [],
          LispyNode.InternalCall("autoReturn", args[0].index, innerScope, args[0].rescope(innerScope)),
          true
        ));
      } else {
        wrapped = args[0];
    ...
    "throw": function (node, args, scope, location, unassigned) {
      var tNode;
    ...
    comment: function (node, args, scope, location, unassigned) {
      var tText;
    ...
    tryCatch: function (node, args, scope, location, unassigned) {
    ...
    tryFinally: function (node, args, scope, location, unassigned) {
    ...
    "for": function (node, args, scope, location, unassigned) {
    ...
    forIn: function (node, args, scope, location, unassigned) {
    ...
    "if": function (node, args, scope, location, unassigned) {
      var innerLocation, k, tTest, tWhenFalse, tWhenTrue, v,
          whenFalseUnassigned;
      if (location === "statement" || location === "topStatement") {
    ...
    array: function (node, args, scope, location, unassigned) {
      var tArr;
      tArr = arrayTranslate(
        getPos(node),
        args,
    ...
    object: function (node, args, scope, location, unassigned) {
    ...
    "switch": function (node, args, scope, location, unassigned) {
      ...
      for (_arr = [], _arr2 = data.cases, _i = 0, _len = _arr2.length; _i < _len; ++_i) {
        case_ = _arr2[_i];
        newCase = {
    ...
    custom: function (node, args, scope, location, unassigned) {
      throw Error("Cannot have a stray custom node '" + args[0].constValue() + "'");
    },
    "super": function (node, args) {
      throw Error("Cannot have a stray super call");
    },
    "var": function (node, args, scope, location, unassigned) {
      var ident, tIdent;
      ident = args[0];
      if (unassigned && !unassigned["\u0000"] && ident instanceof LispyNode.Symbol.ident && !__owns.call(unassigned, ident.name)) {
        unassigned[ident.name] = true;
      }
      tIdent = translate(ident, scope, "leftExpression");
      return function () {
        scope.addVariable(tIdent(), Type.any, args[1] && args[1].constValue());
        return ast.Noop(getPos(node));
      };
    },
    tmpWrapper: function (node, args, scope, location, unassigned) {
  ...
  translateLispyOperator = {
    assign: function (node, args, scope, location, unassigned) {
    ...
    binary: function (node, args, scope, location, unassigned) {
      var tLeft, tRight;
    ...
    unary: function (node, args, scope, location, unassigned) {
      var opName, tSubnode;
      opName = node.func.name;
      if (unassigned && (opName === "++" || opName === "--" || opName === "++post" || opName === "--post") && args[0] instanceof LispyNode.Symbol.ident) {
        unassigned[args[0].name] = false;
      }
  ...
  function translateLispy(node, scope, location, unassigned) {
    ...
    if (node.isValue) {
      ...
      } else if (node.isInternal && node.isNothing) {
        return function () {
          return ast.Noop(getPos(node));
        };
      } else {
      ...
      if (func.isSymbol) {
        if (func.isInternal) {
          name = func.name;
          if (!__owns.call(translateLispyInternal, name)) {
            throw Error("Unable to translate internal call '" + name + "'");
          }
          return translateLispyInternal[name](
            node,
            args,
            scope,
            location,
          ...
          name = func.operatorType;
          if (!__owns.call(translateLispyOperator, name)) {
            throw Error("Unable to translate operator call '" + name + "'");
          }
          return translateLispyOperator[name](
            node,
            args,
            scope,
            location,
  ...
  function translateRoot(roots, scope, getPosition) {
    ...
    if (roots.length === 1) {
      ...
      root = roots[0];
      if (!(root instanceof LispyNode) || !root.isInternalCall("root")) {
      ...
      if (scope.options["return"] || scope.options["eval"]) {
        rootBody = LispyNode.InternalCall("return", rootBody.index, rootBody.scope, rootBody);
      }
