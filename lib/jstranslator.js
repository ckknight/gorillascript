(function () {
  "use strict";
  var __cmp, __create, __instanceofsome, __isArray, __isObject, __num, __owns, __slice, __str, __strnum, __toArray, __typeof, __xor, ast, AstNode, GeneratorBuilder, generatorTranslate, Helpers, HELPERS, ParserNode, Scope, translators, Type;
    ...
    Function: (function () {
      ...
      translateTypeChecks = {
        Ident: function (ident, node, scope, hasDefaultValue, accesses) {
          var access, result;
          access = ast.Access.apply(ast, [ident.pos, ident].concat(__toArray(accesses)));
          scope.addHelper("__typeof");
          result = ast.If(
            ident.pos,
            makeTypeCheckTest(access, node.name, scope),
            ast.Throw(ident.pos, ast.Call(
              ident.pos,
              ast.Ident(ident.pos, "TypeError"),
              [
                ast.BinaryChain.apply(ast, [ident.pos, "+", "Expected " + __strnum(ident.name)].concat(__toArray(buildAccessStringNode(accesses)), [
        ...
        Access: function (ident, node, scope, hasDefaultValue, accesses) {
          ...
          return {
            check: ast.If(
              ident.pos,
              ast.Unary(ident.pos, "!", ast.Binary(ident.pos, access, "instanceof", type)),
              ast.Throw(ident.pos, ast.Call(
                ident.pos,
                ast.Ident(ident.pos, "TypeError"),
                [
        ...
        TypeUnion: function (ident, node, scope, hasDefaultValue, accesses) {
          ...
          result = ast.If(
            ident.pos,
            ast.And.apply(ast, [ident.pos].concat(__toArray(tests))),
            ast.Throw(ident.pos, ast.Call(
              ident.pos,
              ast.Ident(ident.pos, "TypeError"),
              [
            ...
            if (hasNull || hasVoid) {
              if (__xor(hasNull, hasVoid)) {
                result = ast.If(
                  ...
                  ast.Assign(ident.pos, access, ast.Const(ident.pos, hasNull ? null : void 0)),
                  result
                );
              }
            } else if (hasBoolean) {
        ...
        TypeGeneric: function (ident, node, scope, hasDefaultValue, accesses) {
          if (node.basetype.name === "Array") {
            return translateTypeChecks.TypeArray(
              ident,
              { subtype: node.args[0] },
              scope,
              hasDefaultValue,
        ...
        TypeArray: function (ident, node, scope, hasDefaultValue, accesses) {
          ...
          access = ast.Access.apply(ast, [ident.pos, ident].concat(__toArray(accesses)));
          scope.addHelper("__isArray");
          index = scope.reserveIdent(ident.pos, "i", Type.number);
          length = scope.reserveIdent(ident.pos, "len", Type.number);
              ...
              for (_arr = __toArray(node.pairs), _i = 0, _len = _arr.length; _i < _len; ++_i) {
                ...
                if (key instanceof ParserNode.Const) {
                  check = (_ref = translateTypeCheck(
                    ...
                    scope,
                    false,
                    __toArray(accesses).concat([ast.Const(getPos(key), key.value)])
                  )).check;
                  type = _ref.type;
                  typeData[key.value] = type;
                  current = ast.Block(ident.pos, [current, check]);
                }
              }
              return current;
            }())
      ...
      translateParamTypes = {
        Param: function (param, scope, inner) {
          ...
          if (param.defaultValue != null) {
            init.push(ast.If(
              ident.pos,
              ast.Binary(ident.pos, ident, "==", ast.Const(ident.pos, null)),
              ast.Assign(ident.pos, ident, translate(param.defaultValue, scope, "expression")()),
        ...
        Array: function (array, scope, inner) {
          ...
          if (inner) {
            arrayIdent = scope.reserveIdent(getPos(array), "p", Type.array);
          } else {
            arrayIdent = scope.reserveParam(getPos(array));
          }
          init = [];
          foundSpread = -1;
          for (_arr = __toArray(array.elements), i = 0, len = _arr.length; i < len; ++i) {
            ...
            if (!param.spread) {
              if (param.ident != null) {
                if (foundSpread === -1) {
                  init.push(ast.Assign(param.ident.pos, param.ident, ast.Access(param.ident.pos, arrayIdent, i)));
                } else {
                  diff = i - foundSpread - 1;
                  init.push(ast.Assign(param.ident.pos, param.ident, ast.Access(param.ident.pos, arrayIdent, diff === 0 ? spreadCounter : ast.Binary(param.ident.pos, spreadCounter, "+", diff))));
                }
              }
            } else {
              if (foundSpread !== -1) {
              ...
              scope.addHelper("__slice");
              if (i === len - 1) {
                init.push(ast.Assign(param.ident.pos, param.ident, ast.Call(
                  param.ident.pos,
                  ast.Access(
                    param.ident.pos,
                ...
                spreadCounter = scope.reserveIdent(param.ident.pos, "i", Type.number);
                init.push(ast.Assign(param.ident.pos, param.ident, ast.IfExpression(
                  param.ident.pos,
                  ast.Binary(param.ident.pos, i, "<", ast.Assign(param.ident.pos, spreadCounter, ast.Binary(
                    param.ident.pos,
                    ast.Access(param.ident.pos, arrayIdent, "length"),
                    "-",
                    len - i - 1
                  ))),
                  ast.Call(
        ...
        Object: function (object, scope, inner) {
          ...
          if (inner) {
            objectIdent = scope.reserveIdent(getPos(object), "p", Type.object);
          } else {
            objectIdent = scope.reserveParam(getPos(object));
          }
          init = [];
          for (_arr = __toArray(object.pairs), _i = 0, _len = _arr.length; _i < _len; ++_i) {
            ...
            if (value.ident != null) {
              ...
              init.push(ast.Assign(key.pos, value.ident, ast.Access(key.pos, objectIdent, key)));
        ...
        Nothing: function (node, scope, inner) {
          return {
            init: [],
            ident: inner ? null : scope.reserveParam(getPos(node)),
            spread: false
          };
        }
      };
