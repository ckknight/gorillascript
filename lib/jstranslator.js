  ...
  function makeHasGeneratorNode() {
    ...
    function hasGeneratorNode(node, allowReturn) {
      ...
      return (allowReturn ? returnFreeCache : normalCache).getOrAdd(node, (_once = false, function (node) {
        ...
        if (node instanceof LispyNode && node.isCall && node.func.isBreak) {
          return true;
        } else if (node instanceof ParserNode.Yield || node instanceof ParserNode.Continue || !allowReturn && node instanceof ParserNode.Return) {
          return true;
        } else if (!(node instanceof ParserNode.Function)) {
          FOUND = {};
          try {
