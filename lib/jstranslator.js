(function (GLOBAL) {
  ...
  var __arrayToIter, __cmp, __create, __curry, __first, __import,
      ...
      MacroHolder, Map, ParserNode, Scope, translateLispyInternal, translators,
      Type;
    ...
    function hasInSwitch(node) {
      ...
      return inSwitchCache.getOrAdd(node, (_once = false, function (node) {
        ...
        if (!(node instanceof ParserNode.Function)) {
          ...
          try {
            node.walk(function (n) {
              if (n instanceof LispyNode && n.isCall && n.func.isSymbol && n.func.isInternal && (n.func.isForIn || n.func.isFor)) {
                if (hasInLoop(n)) {
                  throw FOUND;
                }
      ...
      return (allowReturn ? returnFreeCache : normalCache).getOrAdd(node, (_once = false, function (node) {
        ...
        if (!(node instanceof ParserNode.Function)) {
          ...
          try {
            node.walk(function (n) {
              var func;
              if (n instanceof LispyNode && n.isCall) {
                func = n.func;
  ...
  function flattenSpreadArray(elements) {
    ...
    for (_arr = __toArray(elements), _i = 0, _len = _arr.length; _i < _len; ++_i) {
      element = _arr[_i];
      if (element instanceof LispyNode && element.isCall && element.func.isSymbol && element.func.isInternal && element.func.isSpread) {
        node = element.args[0];
        if (node instanceof LispyNode && node.isCall && node.func.isSymbol && node.func.isInternal && node.func.isArray) {
  ...
  function parseSwitch(args) {
    var i, lastCaseIndex, len, result;
    ...
    result.defaultCase = args[lastCaseIndex];
    return result;
  }
  function doNothing() {}
  generatorTranslate = (function () {
    ...
    function hasSingleNodeWithNoopsNoSpread(nodes, state) {
      ...
      for (_i = 0, _len = nodes.length; _i < _len; ++_i) {
        node = nodes[_i];
        if (node instanceof LispyNode && node.isCall && node.func.isSymbol && node.func.isInternal && node.func.isSpread) {
          return false;
    ...
    expressions = {
      ...
      9: function (node, scope, state, assignTo, unassigned) {
        ...
        isNew = node.isNew;
        args = node.args;
        if (isApply && (args.length === 0 || !(args[0] instanceof LispyNode) || !args[0].isCall || !args[0].func.isSymbol || !args[0].func.isInternal || !args[0].func.isSpread)) {
          if (args.length === 0) {
            gStart = {
              state: gFunc.state,
              tNode: function () {
    ...
    generatorTranslateExpressionLispyInternals = {
      ...
      block: function (node, args, scope, state, assignTo, unassigned) {
        ...
        for (_arr = __toArray(args), i = 1, len = _arr.length; i < len; ++i) {
          subnode = _arr[i];
          if (i === len - 1) {
    ...
    function isExpression(node) {
      if (node instanceof LispyNode) {
        if (node.isCall && node.func.isInternal) {
    ...
    generatorTranslateLispyInternals = {
      ...
      "return": function (node, args, scope, state, breakState, continueState, unassigned, isTop) {
        ...
        mutatedNode = args[0].mutateLast(
          null,
          function (n) {
            if (n instanceof LispyNode && n.isCall && n.func.isSymbol && n.func.isInternal && n.func.isReturn) {
              return n;
            } else {
  ...
  translateLispyInternal = {
    block: function (node, args, scope, location, unassigned) {
      ...
      tLabel = !(args[0] instanceof ParserNode.Nothing) && translate(args[0], scope, "label");
      for (_arr = [], _arr2 = __toArray(args), i = 1, len = _arr2.length; i < len; ++i) {
        subnode = _arr2[i];
    ...
    tryCatch: function (node, args, scope, location, unassigned) {
      ...
      tLabel = args[3] && translate(args[3], scope, "label");
    ...
    tryFinally: function (node, args, scope, location, unassigned) {
      ...
      tLabel = args[2] && translate(args[2], scope, "label");
      tTryBody = translate(args[0], scope, "statement", unassigned);
    ...
    "for": function (node, args, scope, location, unassigned) {
      ...
      tLabel = args[4] && translate(args[4], scope, "label");
      if (args[0] != null) {
    ...
    "switch": function (node, args, scope, location, unassigned) {
      var _arr, _arr2, _i, _len, baseUnassigned, case_, currentUnassigned, data,
          k, newCase, tCases, tDefaultCase, tLabel, tTopic, v;
      data = parseSwitch(args);
      tLabel = data.label && translate(data.label, scope, "label");
