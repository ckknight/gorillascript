(function (GLOBAL) {
  ...
  function makeHasGeneratorNode() {
    ...
    function hasInSwitch(node) {
      ...
      return inSwitchCache.getOrAdd(node, (_once = false, function (node) {
        ...
        if (node instanceof LispyNode && node.isCall && node.func.isContinue) {
          return true;
        } else if (node instanceof ParserNode.Yield || node instanceof ParserNode.Return) {
          return true;
        } else if (!(node instanceof ParserNode.Function)) {
          FOUND = {};
          try {
    ...
    function hasGeneratorNode(node, allowReturn) {
      ...
      return (allowReturn ? returnFreeCache : normalCache).getOrAdd(node, (_once = false, function (node) {
        ...
        } else if (node instanceof ParserNode.Yield || !allowReturn && node instanceof ParserNode.Return) {
          return true;
        } else if (!(node instanceof ParserNode.Function)) {
          FOUND = {};
          try {
  ...
  function translateRoot(roots, scope, getPosition) {
    ...
    if (roots.length === 1) {
      ...
      ret = translateFunctionBody(rootPos, roots[0].isGenerator, innerScope, scope.options["return"] || scope.options["eval"] ? ParserNode.Return(roots[0].body.index, roots[0].body.scope, roots[0].body) : roots[0].body);
