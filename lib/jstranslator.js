(function (GLOBAL) {
  ...
  function flattenSpreadArray(elements) {
    ...
    for (_arr = __toArray(elements), _i = 0, _len = _arr.length; _i < _len; ++_i) {
      element = _arr[_i];
      if (element instanceof ParserNode.Spread && element.node instanceof LispyNode && element.node.isCall && element.node.func.isSymbol && element.node.func.isInternal && element.node.func.isArray) {
        result.push.apply(result, __toArray(element.node.args));
        changed = true;
      } else {
        result.push(element);
      }
