(function (GLOBAL) {
  ...
  var __async, __create, __curry, __isArray, __keys, __once, __owns, __slice,
      __toArray, __typeof, _ref, AccessNode, AssignNode, BinaryNode, BlockNode,
      CallNode, CustomNode, EmbedWriteNode, FunctionNode, IdentNode, inspect,
      isPrimordial, MacroAccessNode, MacroConstNode, map, mapAsync, Node,
      nodeToType, NothingNode, ParamNode, quote, RootNode, SpreadNode,
  ...
  Node.Access = Node.byTypeId[1] = AccessNode = (function (Node) {
    ...
    _AccessNode_prototype._reduce = function (o) {
      var _arr, _i, _len, _ref, args, cachedParent, child, cValue, end, hasEnd,
          hasStep, inclusive, LispyNode, pair, parent, pValue, start, step,
          value;
      ...
      if (parent.isLiteral() && child.isConst()) {
        ...
        if (parent.isConst()) {
          ...
          if (parent instanceof LispyNode && parent.isCall && parent.func.isSymbol && parent.func.isInternal) {
            if (parent.func.isArray) {
              if (cValue === "length") {
                return LispyNode_Value(this.index, parent.args.length);
              } else if (typeof cValue === "number") {
                return parent.args[cValue] || LispyNode_Value(this.index, void 0);
              }
      ...
      nonConstOps = {
        ...
        "||": function (x, y, o) {
          ...
          if (xType.isSubsetOf(Type.alwaysTruthy)) {
            ...
            LispyNode = require("./parser-lispynodes");
            if (x instanceof LispyNode && x.isCall && x.func.isSymbol && x.func.isInternal && x.func.isIf && x.args[2].isConst() && !x.args[2].constValue()) {
              test = x.args[0];
              whenTrue = x.args[1];
              while (whenTrue instanceof LispyNode && whenTrue.isCall && whenTrue.func.isSymbol && whenTrue.func.isInternal && whenTrue.func.isIf && whenTrue.args[2].isConst() && !whenTrue.args[2].constValue()) {
                test = BinaryNode(
                  x.index,
                  x.scope,
                  test,
  ...
  Node.Custom = Node.byTypeId[14] = CustomNode = (function (Node) {
    var _CustomNode_prototype, _Node_prototype;
    function CustomNode(index, scope, name, data) {
      var _this;
      _this = this instanceof CustomNode ? this : __create(_CustomNode_prototype);
      _this.index = index;
      _this.scope = scope;
      _this._reduced = void 0;
      _this._macroExpanded = void 0;
    ...
    _Node_prototype = Node.prototype;
    _CustomNode_prototype = CustomNode.prototype = __create(_Node_prototype);
    _CustomNode_prototype.constructor = CustomNode;
    CustomNode.displayName = "CustomNode";
    if (typeof Node.extended === "function") {
      Node.extended(CustomNode);
    }
    _CustomNode_prototype.typeId = 14;
    CustomNode.argNames = ["name", "data"];
    _CustomNode_prototype.inspect = function (depth) {
      return inspectHelper(
        depth,
        "CustomNode",
        this.index,
    ...
    _CustomNode_prototype.walk = function (f, context) {
      var data;
      data = map(this.data, f, context);
      if (data !== this.data) {
        return CustomNode(this.index, this.scope, this.name, data);
      } else {
        return this;
      }
    };
    _CustomNode_prototype.walkAsync = function (f, context, callback) {
      var _once, _this;
      _this = this;
      return mapAsync(this.data, f, context, (_once = false, function (_e, data) {
        if (_once) {
          throw Error("Attempted to call function more than once");
        } else {
          _once = true;
        ...
        return callback(null, data !== _this.data ? CustomNode(_this.index, _this.scope, _this.name, data) : _this);
      }));
    };
    return CustomNode;
  }(Node));
