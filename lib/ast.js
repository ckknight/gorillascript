(function () {
  "use strict";
  var __create, __in, __isArray, __lt, __lte, __num, __owns, __slice, __strnum, __toArray, __typeof, Arguments, Arr, Binary, Block, BlockExpression, BlockStatement, Break, Call, Const, Continue, Debugger, DoWhile, Eval, Expression, For, ForIn, Func, getIndent, Ident, If, IfExpression, IfStatement, INDENT, isAcceptableIdent, Level, Node, Noop, Obj, Return, Root, Statement, Switch, This, Throw, toJSSourceTypes, TryCatch, TryFinally, types, Unary, While;
  ...
  exports.Binary = Binary = (function (_super) {
    ...
    function Binary(left, op, right) {
      ...
      if (__owns(LEFT_CONSTANT_OPERATIONS, op) && left.isConst() && (_ref = LEFT_CONSTANT_OPERATIONS[op](left, right)) != null) {
        return _ref;
      }
      if (__owns(RIGHT_CONSTANT_OPERATIONS, op) && right.isConst() && (_ref = RIGHT_CONSTANT_OPERATIONS[op](left, right)) != null) {
        return _ref;
      }
      _this.left = left;
      _this.op = op;
    ...
    RIGHT_CONSTANT_OPERATIONS = {
      ...
      "+": function (x, y) {
        ...
        if (yValue === 0) {
          return Unary("+", x);
        } else if (typeof yValue === "number" && __num(yValue) < 0) {
  ...
  exports.Func = Func = (function (_super) {
    ...
    function Func(name, params, variables, body, declarations, meta) {
      var _i, _len, _ref, _this, item, statement;
      if (name == null) {
        name = null;
  ...
  exports.Unary = Unary = (function (_super) {
    ...
    function Unary(op, node) {
      ...
      if (__owns(NONCONST_OPERATIONS, op) && (_ref = NONCONST_OPERATIONS[op](node)) != null) {
        return _ref;
      }
      _this.op = op;
      _this.node = node;
    ...
    NONCONST_OPERATIONS = {
      ...
      "!": (function () {
        ...
        return function (x) {
          ...
          } else if (x instanceof Binary && __owns(BINARY_INVERTABLE, x.op)) {
            invert = BINARY_INVERTABLE[x.op];
            switch (typeof invert) {
            case "function":
