(function () {
  ...
  nodeToType = (function () {
    ...
    return function (node) {
      ...
      if (!(node instanceof LispyNode)) {
        throw TypeError("Expected a LispyNode, got " + __typeof(node));
      }
      if (node.isValue) {
        switch (node.value) {
        ...
        if (__owns.call(identToType, node.name)) {
        ...
        for (i = 0, _end = +node.args.length; i < _end; i += 2) {
          key = node.args[i];
          if (key instanceof LispyNode && key.isConst()) {
  ...
  function addParamToScope(scope, param, forceMutable) {
    var _arr, _i, asType, child, element, ident, isMutable, isSpread, LispyNode;
    LispyNode = require("./parser-lispynodes");
    ...
    if (param.isInternalCall()) {
      if (param.func.isParam) {
        ...
        if (ident instanceof LispyNode.Symbol.ident || ident instanceof LispyNode.Symbol.tmp) {
          ...
          if (!child.isConstType("string")) {
            throw Error("Expected constant access: " + __typeof(child));
          }
          scope.add(
            LispyNode.Symbol.ident(param.index, param.scope, child.value),
            isMutable,
            asType ? nodeToType(asType) : isSpread ? Type.array : Type.any
          );
        } else {
          throw Error("Unknown param ident: " + __typeof(ident));
        }
