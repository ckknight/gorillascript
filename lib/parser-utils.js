(function () {
  ...
  function addParamToScope(scope, param, forceMutable) {
    var _arr, _i, _ref, child, element, LispyNode, Node;
    Node = require("./parser-nodes");
    LispyNode = require("./parser-lispynodes");
    if (param instanceof Node.Param) {
      if ((_ref = param.ident) instanceof LispyNode.Symbol.ident || _ref instanceof LispyNode.Symbol.tmp) {
        scope.add(param.ident, forceMutable || param.isMutable, param.asType ? nodeToType(param.asType) : param.spread ? Type.array : Type.any);
      } else if (param.ident instanceof LispyNode && param.ident.isInternalCall("access")) {
        child = param.ident.args[1];
