(function () {
  "use strict";
  var __cmp, __create, __in, __isArray, __num, __owns, __slice, __str, __strnum, __toArray, __typeof, __xor, ast, AstNode, GeneratorBuilder, generatorTranslate, Helpers, HELPERS, ParserNode, Scope, translators, Type;
  ...
  Scope = (function () {
    ...
    _proto.maybeCache = function (item, type, func) {
      var ident, result;
    ...
    _proto.fillHelperDependencies = function () {
      var _arr, _else, _i, _len, dep, helper, helpers, toAdd;
      helpers = this.helpers;
      toAdd = {};
      for (; ; ) {
        for (helper in helpers) {
          if (__owns(helpers, helper) && HELPERS.has(helper)) {
            for (_arr = __toArray(HELPERS.dependencies(helper)), _i = 0, _len = _arr.length; _i < _len; ++_i) {
              dep = _arr[_i];
              if (!__owns(helpers, dep)) {
                toAdd[dep] = true;
              }
            }
          }
    ...
    _proto.addVariable = function (ident, type, isMutable) {
      if (!(ident instanceof ast.Ident)) {
        throw TypeError("Expected ident to be a Ident, got " + __typeof(ident));
      }
      if (type == void 0) {
    ...
    _proto.hasOwnVariable = function (ident) {
      if (!(ident instanceof ast.Ident)) {
        throw TypeError("Expected ident to be a Ident, got " + __typeof(ident));
      }
      return __owns(this.variables, ident.name);
    };
    _proto.isVariableMutable = function (ident) {
      var _ref;
      if (!(ident instanceof ast.Ident)) {
        throw TypeError("Expected ident to be a Ident, got " + __typeof(ident));
      }
  ...
  HELPERS = new (Helpers = (function () {
    ...
    _proto.add = function (name, value, dependencies) {
      var _i, _len;
    ...
    _proto.dependencies = function (name) {
      if (typeof name !== "string") {
        throw TypeError("Expected name to be a String, got " + __typeof(name));
      }
      if (__owns(this.deps, name)) {
        ...
        ast.Obj.Pair("next", ast.Func(null, [], [], ast.While(true, ast.TryCatch(
          ast.Switch(
            ...
            (function () {
              ...
              for (_arr = [], _arr2 = __toArray(_this.states), i = 0, _len = _arr2.length, _f = function (i) {
                var state;
                state = _arr2[i];
                return ast.Switch.Case(i, ast.Block(__toArray((function () {
                  var _arr, _arr3, _i, _len, item;
                  for (_arr = [], _arr3 = __toArray(state), _i = 0, _len = _arr3.length; _i < _len; ++_i) {
                    item = _arr3[_i];
                    _arr.push(item());
                  }
                  return _arr;
  ...
  generatorTranslate = (function () {
    ...
    generatorTranslators = {
      ...
      TmpWrapper: function (node, scope, builder, breakState, continueState) {
        ...
        for (_arr = __toArray(node.tmps), _i = 0, _len = _arr.length; _i < _len; ++_i) {
          tmp = _arr[_i];
    ...
    for (_arr = __toArray(flattenSpreadArray(elements)), _i = 0, _len = _arr.length; _i < _len; ++_i) {
      ...
      if (element instanceof ParserNode.Spread) {
        translatedItems.push({
          tNode: translate(element.node, scope, "expression"),
    ...
    if (translatedItems.length === 1) {
      ...
      if (translatedItems.length === 1) {
        ...
        return function () {
          ...
          rest = (function () {
            var _arr, _arr2, _i, _len, item;
            for (_arr = [], _arr2 = __slice(translatedItems, 1, void 0), _i = 0, _len = _arr2.length; _i < _len; ++_i) {
              item = _arr2[_i];
              _arr.push(item());
            }
            return _arr;
  ...
  translators = {
    ...
    Assign: (function () {
      ...
      indexes = {
        slice: function (tParent, parentType, child, tValue, valueType, scope) {
          var left, right, tLeft, tRight;
          left = child.left;
          right = child.right;
          tLeft = left && !(left instanceof ParserNode.Nothing) ? translate(left, scope, "expression")
            : function () {
              return ast.Const(0);
            };
          tRight = right && !(right instanceof ParserNode.Nothing) ? translate(right, scope, "expression")
            : function () {
              return ast.Const(1/0);
            };
          return function () {
            scope.addHelper("__splice");
            return ast.Call(ast.Ident("__splice"), [tParent(), tLeft(), tRight(), tValue()]);
          };
        },
        multi: function (tParent, parentType, child, tValue, valueType, scope, location) {
          var tElements;
          tElements = translateArray(child.elements, scope, "expression");
          return function () {
            return scope.maybeCache(tParent(), parentType, function (setParent, parent) {
              return scope.maybeCache(tValue(), valueType, function (setValue, value) {
                var lines;
                lines = (function () {
                  var _arr, _arr2, _len, i, tElement;
                  for (_arr = [], _arr2 = __toArray(tElements), i = 0, _len = _arr2.length; i < _len; ++i) {
                    tElement = _arr2[i];
      ...
      return function (node, scope, location, autoReturn) {
        var op, result, tLeft, tRight, type;
        op = node.op;
        if (__in(op, "=") && node.left instanceof ParserNode.AccessIndex) {
          type = node.left.child.type;
          if (!__owns(indexes, type)) {
            throw Error("Unexpected index type for assignment: " + __str(JSON.stringify(type)));
          }
          result = indexes[type](
      ...
      translateTypeChecks = {
          ...
          result = ast.If(
            ...
            ast.Throw(ast.Call(ast.Ident("TypeError"), [
              arrayIndex != null
                ? ast.BinaryChain(
                  "+",
                  "Expected " + __strnum(ident.name) + "[",
                  arrayIndex,
                  "] to be a " + __strnum(node.name) + ", got ",
                  ast.Call(ast.Ident("__typeof"), [access])
                )
                : ast.BinaryChain("+", "Expected " + __strnum(ident.name) + " to be a " + __strnum(node.name) + ", got ", ast.Call(ast.Ident("__typeof"), [ident]))
            ]))
          );
              ...
              ast.Throw(ast.Call(ast.Ident("TypeError"), [
                arrayIndex != null
                  ? ast.BinaryChain(
                    ...
                    "Expected " + __strnum(ident.name) + "[",
                    arrayIndex,
                    "] to be a " + __strnum(type.right.value) + ", got ",
                    ast.Call(ast.Ident("__typeof"), [access])
                  )
                  : ast.BinaryChain("+", "Expected " + __strnum(ident.name) + " to be a " + __strnum(type.right.value) + ", got ", ast.Call(ast.Ident("__typeof"), [ident]))
              ]))
            ),
            type: Type.any
          };
        ...
        TypeUnion: function (ident, node, scope, hasDefaultValue, arrayIndex) {
          ...
          result = ast.If(
            ...
            ast.Throw(ast.Call(ast.Ident("TypeError"), [
              ast.BinaryChain("+", "Expected " + __strnum(ident.name) + " to be a " + __strnum(names.join(" or ")) + ", got ", ast.Call(ast.Ident("__typeof"), [ident]))
            ]))
          );
          if (!hasDefaultValue) {
            if (hasNull || hasVoid) {
        ...
        TypeFunction: function (ident, node, scope, hasDefaultValue, arrayIndex) {
        ...
        TypeArray: function (ident, node, scope, hasDefaultValue, arrayIndex) {
          ...
          result = ast.If(
            ...
            ast.Throw(ast.Call(ast.Ident("TypeError"), [
              ast.BinaryChain("+", "Expected " + __strnum(ident.name) + " to be an Array, got ", ast.Call(ast.Ident("__typeof"), [ident]))
            ])),
            ast.For(
              ast.Block([
    ...
    Object: function (node, scope, location, autoReturn) {
      var _arr, _i, _len, pair, tKeys, tPrototype, tValues;
      tKeys = [];
      tValues = [];
      ...
      tPrototype = node.prototype != null ? translate(node.prototype, scope, "expression") : void 0;
      return function () {
        var _arr, _len, constPairs, currentPairs, i, ident, key, obj, postConstPairs, prototype, result, tKey, tValue, value;
        constPairs = [];
        postConstPairs = [];
        prototype = typeof tPrototype === "function" ? tPrototype() : void 0;
        currentPairs = prototype != null ? postConstPairs : constPairs;
        for (_arr = __toArray(tKeys), i = 0, _len = _arr.length; i < _len; ++i) {
          tKey = _arr[i];
          tValue = tValues[i];
          key = tKey();
          value = tValue();
        ...
        obj = prototype != null ? (scope.addHelper("__create"), ast.Call(ast.Ident("__create"), [prototype]))
          : ast.Obj((function () {
            var _arr, _arr2, _i, _len, pair;
            for (_arr = [], _arr2 = __toArray(constPairs), _i = 0, _len = _arr2.length; _i < _len; ++_i) {
              pair = _arr2[_i];
        ...
        if (postConstPairs.length === 0) {
          ...
          result = ast.BlockExpression([ast.Assign(ident, obj)].concat(
            __toArray((function () {
              var _arr, _arr2, _i, _len, pair;
              for (_arr = [], _arr2 = __toArray(postConstPairs), _i = 0, _len = _arr2.length; _i < _len; ++_i) {
                pair = _arr2[_i];
                _arr.push(ast.Assign(
    ...
    Switch: function (node, scope, location, autoReturn) {
      ...
      return function () {
        ...
        return ast.Switch(
          node,
          (function () {
            var _arr, _arr2, case_, caseBody, caseNode, i, len;
            for (_arr = [], _arr2 = __toArray(tCases), i = 0, len = _arr2.length; i < len; ++i) {
              case_ = _arr2[i];
    ...
    Var: function (node, scope, location, autoReturn) {
      var tIdent;
  ...
  function translateArray(nodes, scope, location, autoReturn) {
    ...
    return (function () {
      ...
      for (_arr = [], _arr2 = __toArray(nodes), i = 0, len = _arr2.length; i < len; ++i) {
        node = _arr2[i];
  ...
  module.exports.defineHelper = function (name, value, dependencies) {
    var helper, ident, scope;
    scope = Scope({}, false);
    ident = typeof name === "string" ? ast.Ident(name)
      : name instanceof ParserNode.Ident ? translate(name, scope, "leftExpression")()
      : (function () {
    ...
    helper = value instanceof AstNode ? value
      : value instanceof ParserNode ? translate(value, scope, "expression")()
      : (function () {
