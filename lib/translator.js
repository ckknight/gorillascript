(function () {
  "use strict";
  var __cmp, __create, __in, __isArray, __isObject, __num, __owns, __range, __slice, __str, __strnum, __toArray, __typeof, __xor, ast, AstNode, GeneratorBuilder, generatorTranslate, Helpers, HELPERS, ParserNode, Scope, translators, Type;
  ...
  HELPERS = new (Helpers = (function () {
    ...
    function Helpers() {
      ...
      if (this instanceof Helpers) {
        ...
        _this = __create(_Helpers_prototype);
      }
      _this.data = {};
      _this.types = {};
      _this.deps = {};
  ...
  GeneratorBuilder = (function () {
    ...
    function GeneratorBuilder(scope, states, currentState, stateIdent, pendingFinalliesIdent, finallies, catches, currentCatch) {
      ...
      if (this instanceof GeneratorBuilder) {
        _this = this;
      } else {
        _this = __create(_GeneratorBuilder_prototype);
      }
      _this.scope = scope;
  ...
  function arrayTranslate(elements, scope, replaceWithSlice) {
    ...
    if (translatedItems.length === 1) {
      ...
      if (translatedItems.length === 1) {
        ...
        return function () {
          var _arr, _arr2, _i, _len, head, item, rest;
          head = translatedItems[0]();
          for (_arr = [], _arr2 = __toArray(__slice(translatedItems, 1)), _i = 0, _len = _arr2.length; _i < _len; ++_i) {
            item = _arr2[_i];
            _arr.push(item());
          }
          rest = _arr;
  ...
  translators = {
    Access: function (node, scope, location, autoReturn) {
      ...
      return function () {
        var _ref, child, end, inclusive, parent, slice, start, step;
        parent = tParent();
        child = tChild();
        if (child instanceof ast.Call && child.func instanceof ast.Ident && child.func.name === "__range") {
          ...
          if (!inclusive) {
            inclusive = false;
          } else if (!inclusive.isConst()) {
            throw Error("Expected inclusive argument to be constant");
          } else {
            inclusive = !!inclusive.constValue();
          }
          scope.addHelper("__slice");
          slice = ast.Call(ast.Ident("__slice"), [parent, start].concat(end.isConst() && end.constValue() === 1/0 ? []
            : inclusive
            ? (end.isConst() && typeof end.constValue() === "number" ? (end.constValue() === -1 ? [] : [ast.Const(__num(end.constValue()) + 1)])
              : [
                ast.Binary(
                  ast.Binary(end, "+", 1),
                  "||",
            ...
            : [end]));
          return autoReturn(!step.isConst() || step.constValue() !== 1
            ? (scope.addHelper("__step"), ast.Call(ast.Ident("__step"), [slice, step]))
            : slice);
        } else {
          return autoReturn(ast.Access(parent, child));
        }
      };
    },
    ...
    Assign: (function () {
      ...
      indexes = {
        slice: function (tParent, parentType, child, tValue, valueType, scope) {
          ...
          left = child.left;
          right = child.right;
          if (left && !(left instanceof ParserNode.Nothing)) {
          ...
          if (right && !(right instanceof ParserNode.Nothing)) {
          ...
          return function () {
            return scope.maybeCache(tParent(), parentType, function (setParent, parent) {
              return scope.maybeCache(tValue(), valueType, function (setValue, value) {
                var _arr, _arr2, _len, i, lines, tElement;
    ...
    Function: (function () {
      ...
      function buildAccessStringNode(accesses) {
        if (accesses.length === 0) {
          ...
          return [typeof accesses[0].value === "string" && ast.isAcceptableIdent(accesses[0].value) ? ast.Const("." + __strnum(accesses[0].value)) : ast.Const("[" + __str(JSON.stringify(accesses[0].value)) + "]")].concat(__toArray(buildAccessStringNode(__slice(accesses, 1))));
        } else {
          return ["[", accesses[0], "]"].concat(__toArray(buildAccessStringNode(__slice(accesses, 1))));
        }
      }
      translateTypeChecks = {
    ...
    Object: function (node, scope, location, autoReturn) {
      ...
      if (node.prototype != null) {
        tPrototype = translate(node.prototype, scope, "expression");
      }
      return function () {
    ...
    Switch: function (node, scope, location, autoReturn) {
      var _arr, _arr2, _i, _len, case_, tCases, tDefaultCase, tNode;
      ...
      if (node.defaultCase != null) {
        tDefaultCase = translate(node.defaultCase, scope, "statement");
      }
      return function () {
  ...
  function translateArray(nodes, scope, location, autoReturn) {
    ...
    for (_arr = [], i = 0, len = nodes.length; i < len; ++i) {
      node = nodes[i];
      _arr.push(translate(nodes[i], scope, location, i === len - 1 && autoReturn));
    }
    return _arr;
  }
