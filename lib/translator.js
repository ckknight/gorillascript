...
Scope = (function () {
  ...
  Scope.prototype.hasVariable = function (ident) {
    if (!(ident instanceof ast.Ident)) {
      throw TypeError("Expected ident to be a Ident, got " + __typeof(ident));
    }
...
binaryOps = {
  ...
  "?": function (left, right, scope) {
    if (left instanceof ast.Ident && !scope.hasVariable(left)) {
      return ast.IfExpression(
        ast.Binary(
          ast.Binary(
            ast.Unary("typeof", left),
            "!==",
            ast.Const("undefined")
          ),
          "&&",
  ...
  "?": function (node, scope, location, autoReturn) {
    ...
    if (translated instanceof ast.Ident && !scope.hasVariable(translated)) {
      return autoReturn(ast.Binary(
        ...
        "&&",
        ast.Binary(translated, "!==", ast.Const(null))
      ));
    } else {
      return autoReturn(ast.Binary(translated, "!=", ast.Const(null)));
    }
  },
  "~bitnot": "~",
    ...
    linkTypes = {
      access: (function () {
        ...
        return function (links, link, scope, i, current) {
          var makeAccess;
          makeAccess = link.type === "access"
            ? function (parent) {
              return ast.Access(parent, translate(link.child, scope, "expression"));
            }
            : link.type === "accessIndex"
            ? (!__owns(indexTypes, link.child.type)
              ? (function () {
                throw Error("Unknown index type: " + __strnum(link.child.type));
              }())
              : void 0, indexTypes[link.child.type](link.child, scope))
            : (function () {
      ...
      call: (function () {
        ...
        function makeCall(func, args, isNew, scope) {
          ...
          if (!(argArray instanceof ast.Arr)) {
            return handleSpreadCall(func, argArray, isNew, scope);
          } else {
  ...
  Function: (function () {
    ...
    translateTypeChecks = {
      Ident: function (ident, node, scope, hasDefaultValue, arrayIndex) {
        var access, result;
        access = typeof arrayIndex !== "undefined" && arrayIndex !== null ? ast.Access(ident, arrayIndex) : ident;
        scope.addHelper("__typeof");
        result = ast.If(
          ...
          ast.Throw(ast.Call(ast.Ident("TypeError"), [
            typeof arrayIndex !== "undefined" && arrayIndex !== null
              ? ast.Concat(ast.Const("Expected " + __strnum(ident.name) + "["), arrayIndex, ast.Const("] to be a " + __strnum(node.name) + ", got "), ast.Call(ast.Ident("__typeof"), [access]))
              : ast.Concat(ast.Const("Expected " + __strnum(ident.name) + " to be a " + __strnum(node.name) + ", got "), ast.Call(ast.Ident("__typeof"), [ident]))
          ]))
        );
      ...
      Access: function (ident, node, scope, hasDefaultValue, arrayIndex) {
        ...
        return ast.If(
          ...
          ast.Throw(ast.Call(ast.Ident("TypeError"), [
            typeof arrayIndex !== "undefined" && arrayIndex !== null
              ? ast.Concat(ast.Const("Expected " + __strnum(ident.name) + "["), arrayIndex, ast.Const("] to be a " + __strnum(type.right.value) + ", got "), ast.Call(ast.Ident("__typeof"), [access]))
              : ast.Concat(ast.Const("Expected " + __strnum(ident.name) + " to be a " + __strnum(type.right.value) + ", got "), ast.Call(ast.Ident("__typeof"), [ident]))
          ]))
        );
      },
...
function translate(node, scope, location, autoReturn) {
  ...
  return translators[node.constructor.cappedName](node, scope, location, autoReturn);
}
