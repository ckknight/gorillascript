(function () {
  ...
  function arrayTranslate(elements, scope, replaceWithSlice) {
    ...
    if (translatedItems.length === 1) {
      ...
      for (i = translatedItems.length - 1, _f = function (i) {
        var translatedItem;
  ...
  translators = {
    ...
    Function: (function () {
      ...
      function withArticle(word) {
        return __strnum(article(word)) + " " + __strnum(word);
      }
      translateTypeChecks = {
            ...
            ast.Throw(ast.Call(ast.Ident("TypeError"), [
              arrayIndex != null
                ? ast.BinaryChain(
                  ...
                  arrayIndex,
                  "] to be " + __strnum(withArticle(node.name)) + ", got ",
                  ast.Call(ast.Ident("__typeof"), [access])
                )
                : ast.BinaryChain("+", "Expected " + __strnum(ident.name) + " to be " + __strnum(withArticle(node.name)) + ", got ", ast.Call(ast.Ident("__typeof"), [ident]))
            ]))
          );
              ...
              ast.Throw(ast.Call(ast.Ident("TypeError"), [
                arrayIndex != null
                  ? ast.BinaryChain(
                    ...
                    "Expected " + __strnum(ident.name) + "[",
                    arrayIndex,
                    "] to be " + __strnum(withArticle(type.right.value)) + ", got ",
                    ast.Call(ast.Ident("__typeof"), [access])
                  )
                  : ast.BinaryChain("+", "Expected " + __strnum(ident.name) + " to be " + __strnum(withArticle(type.right.value)) + ", got ", ast.Call(ast.Ident("__typeof"), [ident]))
              ]))
            ),
            type: Type.any
          };
        ...
        TypeUnion: function (ident, node, scope, hasDefaultValue, arrayIndex) {
          ...
          result = ast.If(
            ...
            ast.Throw(ast.Call(ast.Ident("TypeError"), [
              ast.BinaryChain("+", "Expected " + __strnum(ident.name) + " to be " + __strnum(withArticle(names.join(" or "))) + ", got ", ast.Call(ast.Ident("__typeof"), [ident]))
            ]))
          );
          if (!hasDefaultValue) {
            if (hasNull || hasVoid) {
    ...
    Object: function (node, scope, location, autoReturn) {
      ...
      return function () {
        var _len, constPairs, currentPairs, i, ident, key, obj, postConstPairs, prototype, result, tKey, tValue, value;
        constPairs = [];
        postConstPairs = [];
