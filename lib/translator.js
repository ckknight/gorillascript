...
translators = {
  ...
  array: function (node, scope, location, autoReturn) {
    var _tmp, _tmp2, _tmp3, array, current, element, i, translatedItem, translatedItems;
    translatedItems = [];
    current = [];
    translatedItems.push(current);
    for (_tmp = node.elements, _tmp2 = 0, _tmp3 = __num(_tmp.length); _tmp2 < _tmp3; ++_tmp2) {
      element = _tmp[_tmp2];
      ...
      multi: function (parent, child, value, scope, location) {
        return scope.maybeCache(parent, function (setParent, parent) {
          return scope.maybeCache(value, function (setValue, value) {
            ...
            lines = (function () {
              var _tmp, _tmp2, element, i;
              for (_tmp = [], i = 0, _tmp2 = __num(elements.length); i < _tmp2; ++i) {
                element = elements[i];
                _tmp.push(ast.Assign(ast.Access(i === 0 ? setParent : parent, element), ast.Access(i === 0 ? setValue : value, ast.Const(i))));
              }
              return _tmp;
            }());
            if (location === "expression") {
    ...
    linkTypes = {
            ...
            return function (parent) {
              return scope.maybeCache(parent, function (setParent, parent) {
                ...
                return ast.Arr((function () {
                  ...
                  for (_tmp = [], i = 0, _tmp2 = __num(elements.length); i < _tmp2; ++i) {
                    element = elements[i];
                    _tmp.push(ast.Access(i === 0 ? setParent : parent, element));
                  }
                  return _tmp;
                }()));
              });
      ...
      call: (function () {
        ...
        function makeCall(func, args, isNew, scope) {
          ...
          for (_tmp = 0, _tmp2 = __num(args.length); _tmp < _tmp2; ++_tmp) {
            arg = args[_tmp];
            if (arg.type === "spread") {
              current = [];
  ...
  defineHelper: function (node, scope, location, autoReturn) {
    ...
    module.exports.knownHelpers.push(ident.name);
  ...
  "function": (function () {
    ...
    translateTypeChecks = {
      ident: function (ident, node, scope, hasDefaultValue, arrayIndex) {
        ...
        scope.addHelper("__typeof");
        result = ast.If(makeTypeCheckTest(access, node.name, scope), ast.Throw(ast.Call(ast.Ident("TypeError"), [
          arrayIndex != null ? ast.Concat(ast.Const("Expected " + __strnum(ident.name) + "["), arrayIndex, ast.Const("] to be a " + __strnum(node.name) + ", got "), ast.Call(ast.Ident("__typeof"), [access])) : ast.Concat(ast.Const("Expected " + __strnum(ident.name) + " to be a " + __strnum(node.name) + ", got "), ast.Call(ast.Ident("__typeof"), [ident]))
        ])));
      ...
      typeUnion: function (ident, node, scope, hasDefaultValue, arrayIndex) {
        var _tmp, _tmp2, _tmp3, check, hasBoolean, hasNull, hasVoid, names, result, tests, type;
        if (arrayIndex != null) {
        ...
        names = [];
        tests = [];
        for (_tmp = node.types, _tmp2 = 0, _tmp3 = __num(_tmp.length); _tmp2 < _tmp3; ++_tmp2) {
          type = _tmp[_tmp2];
          if (type.type === "const") {
            if (type.value === null) {
              hasNull = true;
    ...
    translateParamTypes = {
      ...
      array: function (array, scope, inner) {
        ...
        for (_tmp = array.elements, i = 0, len = __num(_tmp.length); i < len; ++i) {
          p = _tmp[i];
      ...
      object: function (object, scope, inner) {
        var _tmp, _tmp2, _tmp3, init, key, objectIdent, pair, value;
        objectIdent = inner ? scope.reserveIdent() : scope.reserveParam();
        init = [];
        for (_tmp = object.pairs, _tmp2 = 0, _tmp3 = __num(_tmp.length); _tmp2 < _tmp3; ++_tmp2) {
          pair = _tmp[_tmp2];
    ...
    return function (node, scope, location, autoReturn) {
      ...
      foundSpread = -1;
      spreadCounter = void 0;
      for (_tmp = node.params, i = 0, len = __num(_tmp.length); i < len; ++i) {
        p = _tmp[i];
  ...
  object: function (node, scope, location, autoReturn) {
    var _tmp, _tmp2, _tmp3, constPairs, currentPairs, ident, key, obj, pair, postConstPairs, result, value;
    constPairs = [];
    postConstPairs = [];
    currentPairs = constPairs;
    for (_tmp = node.pairs, _tmp2 = 0, _tmp3 = __num(_tmp.length); _tmp2 < _tmp3; ++_tmp2) {
      pair = _tmp[_tmp2];
      key = translate(pair.key, scope, "expression");
    ...
    obj = ast.Obj((function () {
      var _tmp, _tmp2, _tmp3, pair;
      for (_tmp = [], _tmp2 = 0, _tmp3 = __num(constPairs.length); _tmp2 < _tmp3; ++_tmp2) {
        pair = constPairs[_tmp2];
        _tmp.push(ast.Obj.Pair(String(pair.key.value), pair.value));
      }
      return _tmp;
    }()));
    if (postConstPairs.length === 0) {
      ...
      result = ast.BlockExpression([ast.Assign(ident, obj)].concat(
        __toArray((function () {
          var _tmp, _tmp2, _tmp3, pair;
          for (_tmp = [], _tmp2 = 0, _tmp3 = __num(postConstPairs.length); _tmp2 < _tmp3; ++_tmp2) {
            pair = postConstPairs[_tmp2];
            _tmp.push(ast.Assign(ast.Access(ident, pair.key), pair.value));
          }
          return _tmp;
        }())),
        [ident]
  ...
  root: function (node, scope) {
    ...
    for (_tmp = scope.getHelpers(), _tmp2 = 0, _tmp3 = __num(_tmp.length); _tmp2 < _tmp3; ++_tmp2) {
      helper = _tmp[_tmp2];
  ...
  string: function (node, scope, location, autoReturn) {
    ...
    parts = node.parts;
    current = void 0;
    for (_tmp = 0, _tmp2 = __num(parts.length); _tmp < _tmp2; ++_tmp) {
      p = parts[_tmp];
  ...
  tmp: function (node, scope, location, autoReturn) {
    return autoReturn(scope.getTmp(node.id));
  },
  ...
  useMacro: function (node, scope, location, autoReturn) {
    ...
    for (_tmp = node.tmps, _tmp2 = 0, _tmp3 = __num(_tmp.length); _tmp2 < _tmp3; ++_tmp2) {
      tmp = _tmp[_tmp2];
      scope.releaseTmp(tmp);
    }
    for (_tmp = node.macroHelpers, _tmp2 = 0, _tmp3 = __num(_tmp.length); _tmp2 < _tmp3; ++_tmp2) {
      helper = _tmp[_tmp2];
...
function translateArray(nodes, scope, location, autoReturn) {
  ...
  return (function () {
    var _tmp, i, len, node;
    for (_tmp = [], i = 0, len = __num(nodes.length); i < len; ++i) {
      node = nodes[i];
