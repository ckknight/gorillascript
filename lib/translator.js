(function () {
  ...
  function arrayTranslate(elements, scope, replaceWithSlice, allowArrayLike) {
  ...
  translators = {
    ...
    Array: function (node, scope, location, autoReturn) {
      var tArr;
      tArr = arrayTranslate(node.elements, scope, true, false);
      return function () {
    ...
    Call: function (node, scope, location, autoReturn) {
      ...
      if (isApply && (args.length === 0 || !(args[0] instanceof ParserNode.Spread))) {
        ...
        tArgArray = arrayTranslate(args, scope, false, true);
        return function () {
          var argArray, func;
          func = tFunc();
          argArray = tArgArray();
