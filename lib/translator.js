...
translators = {
    ...
    if (translatedItems.length === 1) {
      ...
      if (translatedItems.length === 1) {
        ...
        return autoReturn(ast.Call(
          ast.Access(translatedItems[0], ast.Const("concat")),
    ...
    indexes = {
      ...
      slice: function (parent, child, value, scope) {
        ...
        return ast.Call(ast.Ident("__splice"), [
          parent,
          left && left.type !== "nothing" ? translate(left, scope, "expression") : ast.Const(0),
          right && right.type !== "nothing" ? translate(right, scope, "expression") : ast.Const(1/0),
          value
        ]);
      },
      ...
      call: (function () {
        ...
        function makeCall(func, args, isNew, scope) {
          ...
          if (translatedArgs.length === 1) {
            ...
            array = translatedArgs.length === 1 ? translatedArgs[0] : ast.Call(
              ast.Access(translatedArgs[0], ast.Const("concat")),
              __slice(translatedArgs, 1, void 0)
            );
          ...
          if (!link.existential) {
            ...
            return scope.maybeCache(current, function (setFunc, func) {
              return ast.If(
                ...
                handle(links, scope, __num(i) + 1, makeCall(func, link.args, link.isNew, scope))
              );
            });
          }
        };
    ...
    translateTypeChecks = {
      ident: function (ident, node, scope, hasDefaultValue, arrayIndex) {
        ...
        result = ast.If(
          ...
          ast.Throw(ast.Call(ast.Ident("TypeError"), [arrayIndex != null ? ast.Concat(ast.Const("Expected " + __strnum(ident.name) + "["), arrayIndex, ast.Const("] to be a " + __strnum(node.name) + ", got "), ast.Call(ast.Ident("__typeof"), [access])) : ast.Concat(ast.Const("Expected " + __strnum(ident.name) + " to be a " + __strnum(node.name) + ", got "), ast.Call(ast.Ident("__typeof"), [ident]))]))
        );
      ...
      access: function (ident, node, scope, hasDefaultValue, arrayIndex) {
        ...
        return ast.If(
          ...
          ast.Throw(ast.Call(ast.Ident("TypeError"), [arrayIndex != null ? ast.Concat(ast.Const("Expected " + __strnum(ident.name) + "["), arrayIndex, ast.Const("] to be a " + __strnum(type.right.value) + ", got "), ast.Call(ast.Ident("__typeof"), [access])) : ast.Concat(ast.Const("Expected " + __strnum(ident.name) + " to be a " + __strnum(type.right.value) + ", got "), ast.Call(ast.Ident("__typeof"), [ident]))]))
        );
      },
      typeUnion: function (ident, node, scope, hasDefaultValue, arrayIndex) {
        ...
        result = ast.If(
          ...
          ast.Throw(ast.Call(ast.Ident("TypeError"), [ast.Concat(ast.Const("Expected " + __strnum(ident.name) + " to be a " + __strnum(names.join(" or ")) + ", got "), ast.Call(ast.Ident("__typeof"), [ident]))]))
        );
        if (!hasDefaultValue) {
          if (hasNull || hasVoid) {
          ...
          if (!param.spread) {
            ...
            if (i === __num(len) - 1) {
              ...
              init.push(ast.Assign(param.ident, ast.IfExpresion(
                ast.Binary(ast.Const(i), "<", ast.Assign(spreadCounter, ast.Binary(
                  ast.Binary(arrayIdent, ".", ast.Const("length")),
                  "-",
                  ast.Const(__num(len) - __num(i) - 1)
                ))),
  ...
  macroHelper: function (node, scope, location, autoReturn) {
    ...
    return autoReturn(ast.Obj([
      ast.Obj.Pair("type", ast.Const("ident")),
      ast.Obj.Pair("name", ast.Const(name))
    ]));
  },
  nothing: function () {
    return ast.BlockExpression();
  ...
  operator: (function () {
    ...
    operators = {
      ...
      bitnot: function (scope) {
        var left;
        left = ast.Ident("x");
        return ast.Func(null, [left], [], [ast.Return(ast.Unary("~", numWrap(left, scope)))]);
      },
      bool: function (scope) {
        var value;
        value = ast.Ident("x");
        return ast.Func(null, [value], [], [ast.Return(ast.Unary("!", ast.Unary("!", value)))]);
      },
      ...
      "throw?": function (scope) {
        var error;
        error = ast.Ident("e");
        return ast.Func(null, [error], [], [ast.IfStatement(
    ...
    return function (node, scope, location, autoReturn) {
      ...
      if (__owns(operators, op)) {
        ...
        value = ast.Ident("x");
        return autoReturn(ast.Func(null, [value], [], [ast.Return(ast.Unary(unaryOps[op], value))]));
      } else {
    ...
    if (node.existential) {
      return scope.maybeCache(value, function (setValue, value) {
        return ast.IfStatement(
          ast.Binary(setValue, "!=", ast.Const(null)),
          ast.Return(value)
        );
      });
    } else {
