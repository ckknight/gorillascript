  ...
  assign: (function () {
    ...
    ops = {
      ...
      "~&=": function (left, right, scope) {
        if (left.type().overlaps(types.number) && right.type().overlaps(types.number)) {
          return scope.maybeCacheAccess(left, function (setLeft, left) {
            return ast.Binary(setLeft, "+=", ast.Binary(
              ast.Const(""),
              "+",
              right
            ));
      ...
      "xor=": function (left, right, scope) {
        scope.addHelper("__xor");
        return scope.maybeCacheAccess(left, function (setLeft, left) {
          return ast.Assign(setLeft, ast.Call(
            ast.Ident("__xor"),
            [left, right]
          ));
        });
            ...
            lines = (function () {
              ...
              for (_arr = [], i = 0, _len = __num(elements.length); i < _len; ++i) {
                ...
                _arr.push(ast.Assign(
                  ast.Access(i === 0 ? setParent : parent, element),
                  ast.Access(i === 0 ? setValue : value, ast.Const(i))
                ));
              }
              return _arr;
            }());
  ...
  "function": (function () {
    ...
    translateTypeChecks = {
      ident: function (ident, node, scope, hasDefaultValue, arrayIndex) {
        ...
        result = ast.If(
          ...
          ast.Throw(ast.Call(
            ast.Ident("TypeError"),
            [arrayIndex != null ? ast.Concat(
              ast.Const("Expected " + __strnum(ident.name) + "["),
              arrayIndex,
              ast.Const("] to be a " + __strnum(node.name) + ", got "),
              ast.Call(
                ast.Ident("__typeof"),
                [access]
              )
            ) : ast.Concat(
              ast.Const("Expected " + __strnum(ident.name) + " to be a " + __strnum(node.name) + ", got "),
              ast.Call(
                ast.Ident("__typeof"),
                [ident]
              )
        ...
        return ast.If(
          ...
          ast.Throw(ast.Call(
            ...
            [arrayIndex != null ? ast.Concat(
              ast.Const("Expected " + __strnum(ident.name) + "["),
              arrayIndex,
              ast.Const("] to be a " + __strnum(type.right.value) + ", got "),
              ast.Call(
                ast.Ident("__typeof"),
                [access]
              )
            ) : ast.Concat(
              ast.Const("Expected " + __strnum(ident.name) + " to be a " + __strnum(type.right.value) + ", got "),
              ast.Call(
                ast.Ident("__typeof"),
                [ident]
              )
      ...
      typeUnion: function (ident, node, scope, hasDefaultValue, arrayIndex) {
        ...
        result = ast.If(
          ...
          ast.Throw(ast.Call(
            ast.Ident("TypeError"),
            [ast.Concat(
              ast.Const("Expected " + __strnum(ident.name) + " to be a " + __strnum(names.join(" or ")) + ", got "),
              ast.Call(
                ast.Ident("__typeof"),
                [ident]
              )
      ...
      typeArray: function (ident, node, scope, hasDefaultValue, arrayIndex) {
        ...
        return ast.If(
          ...
          ast.Throw(ast.Call(
            ast.Ident("TypeError"),
            [ast.Concat(
              ast.Const("Expected " + __strnum(ident.name) + " to be an Array, got "),
              ast.Call(
                ast.Ident("__typeof"),
                [ident]
              )
    ...
    translateParamTypes = {
      param: function (param, scope, inner) {
        ...
        return {
          init: __toArray(init).concat(__toArray(laterInit)),
          ident: ident,
          spread: !!param.spread
        };
      },
            ...
            if (i === __num(len) - 1) {
              ...
              init.push(ast.Assign(param.ident, ast.IfExpresion(
                ast.Binary(
                  ast.Const(i),
                  "<",
                  ast.Assign(spreadCounter, ast.Binary(
