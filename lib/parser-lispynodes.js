(function () {
  ...
  var __async, __create, __is, __isArray, __once, __owns, __slice, __toArray,
      __typeof, Cache, Call, Node, OldNode, Symbol, toJSSource, Type, Value;
        ...
        block: {
              ...
              if (_value === void 0) {
                _value = (function () {
                  ...
                  if (args.length === 0) {
                    ...
                    for (_arr = __toArray(args), _i = 1, _len = _arr.length, _end = -1, _end += _len, _end > _len && (_end = _len); _i < _end; ++_i) {
                      node = _arr[_i];
                      node.type(parser);
                    }
          ...
          _withLabel: function (call, label, parser) {
            ...
            if (args[0] instanceof OldNode.Nothing) {
              len = args.length;
              if (len === 2) {
                return args[1].withLabel(label(parser));
              } else if (len > 2) {
                last = args[len - 1];
                if (last instanceof Node && last.isCall && last.func.isInternal && last.func.isForIn && (function () {
                  var _arr, _end, _every, _i, _len, node;
                  _every = true;
                  for (_arr = __toArray(args), _i = 1, _len = _arr.length, _end = -1, _end += _len, _end > _len && (_end = _len); _i < _end; ++_i) {
                    node = _arr[_i];
                    if (!(node instanceof OldNode.Assign) && (!(node instanceof Node) || !node.isCall || !node.func.isSymbol || !node.func.isInternal || !node.func.isVar)) {
                      _every = false;
                      break;
                    }
                  }
          ...
          __reduce: function (call, parser) {
            var _arr, _arr2, _i, _len, arg, args, body, changed, i, label, len,
                node, reduced;
            changed = false;
            body = [];
            args = call.args;
            label = args[0].reduce(parser);
            for (_arr = __toArray(args), i = 1, len = _arr.length; i < len; ++i) {
              node = _arr[i];
              reduced = node.reduce(parser);
              if (reduced instanceof OldNode.Nothing) {
                changed = true;
              } else if (reduced instanceof Node && reduced.isCall && reduced.func.isSymbol && reduced.func.isInternal) {
                if (reduced.func.isBlock && reduced.args[0] instanceof OldNode.Nothing) {
                  for (_arr2 = __toArray(reduced.args), _i = 1, _len = _arr2.length; _i < _len; ++_i) {
                    arg = _arr2[_i];
                    body.push(arg);
                  }
                  changed = true;
            ...
            } else if (label instanceof OldNode.Nothing && body.length === 1) {
              return body[0];
            } else if (changed || label !== args[0]) {
              return Call.apply(void 0, [this.index, this.scope, call.func, label].concat(__toArray(body)));
            } else {
              return call;
            }
          },
