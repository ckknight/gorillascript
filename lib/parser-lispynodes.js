    ...
    Internal = (function (Symbol) {
      ...
      internalSymbols = {
            ...
            function replaceLengthIdent(node) {
              ...
              if (node instanceof Ident && node.name === "__currentArrayLength") {
                ...
                if (node instanceof Node && node.isInternalCall("access")) {
                  nodeParent = replaceLengthIdent(node.args[0].reduce(parser));
        ...
        block: {
          ...
          __reduce: function (call, parser) {
            var _arr, args, body, changed, i, len, node;
            changed = false;
            body = [];
            args = call.args;
        ...
        "if": {
          ...
          __reduce: function (call, parser) {
            var _ref, test, testType, whenFalse, whenTrue;
            _ref = call.args;
            test = _ref[0];
            whenTrue = _ref[1];
            whenFalse = _ref[2];
    ...
    _Call_prototype.doWrap = function (parser) {
      ...
      if (typeof this.func._doWrap === "function") {
        ...
        result = Call(this.index, this.scope, OldNode.Function(
          ...
          this.scope,
          [],
          Call(this.index, innerScope, Symbol.autoReturn(this.index), this.rescope(innerScope)),
          false,
          true
        ));
        parser.popScope();
