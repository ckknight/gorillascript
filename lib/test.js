(function (GLOBAL) {
  "use strict";
  var __async, __create, __in, __is, __isArray, __num, __once, __owns, __slice, __strnum, __toArray, __typeof, argv, assert, currentFile, fs, gorilla, k, noPrelude, numFailures, options, passedTests, path, totalTime, waiters;
      ...
      return __async(
        ...
        function (err) {
          ...
          function next() {
            ...
            write("     parse     macro     reduce    translate compile " + (options.uglify ? "  uglify  " : "") + "  eval            total\n");
            totals = {};
            return __async(
              1,
              __num(files.length),
              function (_i, next) {
                var _o, _ref, basename, code, failure, file, filename, start, startTime;
                file = files[_i];
                ...
                return gorilla["eval"](
                  code.toString(),
                  (_o = __create(options), _o.filename = filename, _o.includeGlobals = true, _o.noPrelude = noPrelude, _o.progress = progress, _o),
                  function (err, result) {
                    var endTime;
                    if (err != null) {
                      write("\n");
