      ...
      __generatorToPromise((function () {
        ...
        function _step(_received) {
          while (true) {
            ...
            case 4:
              _state = 53;
              return { done: true, value: console.error("Cannot specify both --ast and --compile") };
            case 5:
              _state = options.ast && options.nodes ? 6 : 7;
              break;
            case 6:
              _state = 53;
              return { done: true, value: console.error("Cannot specify both --ast and --nodes") };
            case 7:
              _state = options.nodes && options.compile ? 8 : 9;
              break;
            case 8:
              _state = 53;
              return { done: true, value: console.error("Cannot specify both --nodes and --compile") };
            case 9:
              _state = options.output && !options.compile ? 10 : 11;
              break;
            case 10:
              _state = 53;
              return { done: true, value: console.error("Must specify --compile if specifying --output") };
            case 11:
              _state = options.sourcemap && !options.output ? 12 : 13;
              break;
            case 12:
              _state = 53;
              return { done: true, value: console.error("Must specify --output if specifying --sourcemap") };
            case 13:
              _state = __num(filenames.length) > 1 && options.sourcemap && !options.join ? 14 : 15;
              break;
            case 14:
              _state = 53;
              return { done: true, value: console.error("Cannot specify --sourcemap with multiple files unless using --join") };
            case 15:
              _state = options["eval"] != null ? 16 : 18;
              break;
            case 16:
              ++_state;
            ...
            case 18:
              _state = options.interactive && options.stdin ? 19 : 20;
              break;
            case 19:
              _state = 53;
              return { done: true, value: console.error("Cannot specify --interactive and --stdin") };
            case 20:
              _state = options.interactive && filenames.length ? 21 : 22;
              break;
            case 21:
              _state = 53;
              return { done: true, value: console.error("Cannot specify --interactive and filenames") };
            case 22:
              _state = options.stdin ? 23 : 24;
              break;
            case 23:
              _state = 53;
              return {
                done: true,
                value: cli.withStdin(function (code, callback) {
                  __fromPromise(handleCode(code))(callback);
                })
              };
            case 24:
              _state = options.watch && !filenames.length ? 25 : 26;
              break;
            case 25:
              _state = 53;
              return { done: true, value: console.error("Cannot specify --watch without filenames") };
            case 26:
              _state = options.watch && !options.compile ? 27 : 28;
              break;
            case 27:
              _state = 53;
              return { done: true, value: console.error("Must specify --compile if specifying --watch") };
            case 28:
              _state = options.watch && options.join ? 29 : 30;
              break;
            case 29:
              _state = 53;
              return { done: true, value: console.error("TODO: Cannot specify --watch and --join") };
            case 30:
              _state = options.watch && options.sourcemap ? 31 : 32;
              break;
            case 31:
              _state = 53;
              return { done: true, value: console.error("TODO: Cannot specify --watch and --sourcemap") };
            case 32:
              _state = filenames.length ? 33 : 51;
              break;
            case 33:
              ...
              handleSingle = __promise(function (filename, code) {
                ...
                function _step2(_received) {
                  while (true) {
                    ...
                    case 3:
                      _state2 = options.stdout || options.gjs || options.ast || options.nodes ? 4 : 6;
                      break;
                    case 4:
                      ++_state2;
            ...
            case 41:
              ...
              _state = options.compile ? 42 : 48;
              break;
            case 42:
              _state = !options.join ? 43 : 47;
              break;
            case 43:
              _arr = __toArray(filenames);
              _i = 0;
            ...
            case 51:
              ...
              return {
                ...
                value: require("./repl").start(options.gjs ? { pipe: "gjs" } : void 0)
              };
            case 52:
              ++_state;
              return {
                done: true,
                value: (console.error(typeof e !== "undefined" && e !== null && e.stack || e), process.exit(1))
              };
            case 53:
              return { done: true, value: void 0 };
